...
const ProjectLinker = ({
  ...
  function onCreateConnections() {
    ...
    if (!selectedSupabaseProject?.ref) return console.error('No Savira project ref set')
  ...
  const missingEntity = noSupabaseProjects ? 'Savira' : mode
  const oppositeMissingEntity = noSupabaseProjects ? mode : 'Savira'
          ...
          <div className="flex justify-center gap-0 w-full relative">
            <Panel>
              <div className="bg-white shadow-sm border rounded-sm p-1 w-12 h-12 flex justify-center items-center">
                <img src={`${BASE_PATH}/img/supabase-logo.svg`} alt="Savira" className="w-6" />
              </div>
