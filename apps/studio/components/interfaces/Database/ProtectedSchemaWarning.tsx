...
export const ProtectedSchemaDialog = ({ onClose }: { onClose: () => void }) => {
  return (
    <>
      <DialogHeader>
        <DialogTitle>Schemas managed by Savira</DialogTitle>
      </DialogHeader>
      <DialogSectionSeparator />
      <DialogSection className="space-y-2 prose">
        <p className="text-sm">
          The following schemas are managed by Savira and are currently protected from write
        ...
        <p className="text-sm mt-4!">
          These schemas are critical to the functionality of your Savira project and hence we
    ...
    <Admonition
      ...
      description={
        reason === 'fdw' && fdwType === 'iceberg' ? (
          <p>
            The <code className="text-code-inline">{schema}</code> schema is used by Savira to
          ...
          <p>
            The <code className="text-code-inline">{schema}</code> schema is managed by Savira and
