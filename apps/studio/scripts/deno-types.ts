...
async function downloadSupabaseFunctionsJsTypes() {
  console.log('Downloading Savira Functions JS types')
  ...
  try {
    ...
    console.log('Savira Functions JS types downloaded successfully')
  } catch (error) {
    console.error('Error downloading Savira Functions JS types', error)
    process.exit(1)
  }
}
