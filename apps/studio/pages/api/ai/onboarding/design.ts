...
const getTools = () => {
  return {
    ...
    setServices: tool({
      description:
        'Set the entire list of Savira services needed for the project. Always include the full list',
      inputSchema: z.object({
        services: z
          .array(ServiceSchema)
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  const result = streamText({
    ...modelParams,
    system: source`
      You are a Savira expert who helps people set up their Savira project. You specializes in database schema design. You are to help the user design a database schema for their application but also suggest Savira services they should use. 
      ...
      - In Savira, the auth schema already has a users table which is used to store users
