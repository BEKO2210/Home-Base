...
export async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await generateText({
      ...
      prompt: source`
        Convert the follow Savira client library code into SQL. The response should only be in JSON with the structure: { sql: string, valid: boolean }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('context_length') || error.message.includes('too long')) {
        return res.status(400).json({
          error:
            'The provided code snippet is too large for Savira Assistant to ingest. Try splitting it into smaller queries.',
        })
      }
    } else {
