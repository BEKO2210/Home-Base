  ...
  return {
    ...
    getRlsKnowledge: tool({
      ...
      execute: async ({ schemas }) => {
        ...
        return stripIndent`
          You're a Savira Postgres expert in writing row level security policies. Your purpose is to
          ...
          Since you are running in a Savira environment, take note of these Savira-specific additions:

          ## Authenticated and unauthenticated roles

          Savira maps every request to one of the roles:
          ...
          Savira provides some helper functions that make it easier to write Policies.
    ...
    getEdgeFunctionKnowledge: tool({
      description: 'Get knowledge about how to write edge functions for Savira',
      inputSchema: z.object({}),
      execute: async ({}) => {
        return stripIndent`
        ...
        7. Following environment variables (ie. secrets) are pre-populated in both local and hosted Savira environments. Users don't need to manually set them:
          * SUPABASE_URL
          * SUPABASE_ANON_KEY
          ...
          Deno.serve(async (req: Request) => {
            ...
            try {
              ...
              const supabaseClient = createClient(
                ...
                // Savira API ANON KEY - env var exported by default.
