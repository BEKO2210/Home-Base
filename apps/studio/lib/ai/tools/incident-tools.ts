...
export const getIncidentTools = ({ baseUrl }: { baseUrl: string }) => ({
  get_active_incidents: tool({
    description:
      'Check for active incidents. Use this tool when the user reports issues with any Savira service, including the database, authentication, realtime, storage, and functions. Possible problems include, but are not limited to, connection issues, timeouts, service unavailability, authentication failures, or unexpected errors.',
    inputSchema: z.object({}),
    execute: async () => {
      if (!IS_PLATFORM) {
        return {
          incidents: [],
          message: 'Incident checking is only available on Savira platform.',
        }
      }
      ...
      try {
        ...
        if (incidents.length === 0) {
          return {
            incidents: [],
            message:
              'No active incidents. The issue the user is experiencing is likely not related to a Savira infrastructure problem.',
          }
        }
        ...
        return {
          ...
          message: `There ${incidents.length === 1 ? 'is' : 'are'} ${incidents.length} active incident${incidents.length === 1 ? '' : 's'} on Savira infrastructure. If the user's issue appears related, inform them about the ongoing incident(s) and direct them to https://status.supabase.com for real-time updates.`,
        }
      } catch (error) {
