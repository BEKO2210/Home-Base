...
The component creates a [Yjs](https://yjs.dev/) document and connects it to a Savira Realtime channel using `SupabaseProvider` from `@supabase-labs/y-supabase`. A `MonacoBinding` from `y-monaco` bridges the Yjs document with the Monaco editor model, keeping them in sync.
...
Enable persistence to save the editor content to your Savira database. This requires a table to store the Yjs document state.
...
| `persistence?` | `boolean \| SupabasePersistenceOptions` | Persists editor content to Savira so it survives page reloads. Pass `true` for defaults or an options object for fine-grained control.        |
