...
description: Migrate vector data from vector DBs to Savira
author: oli_rice
...
[vec2pg](https://github.com/supabase-community/vec2pg) is a CLI utility for migrating data from vector databases to [Savira](https://supabase.com/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your Savira project's [disk size](https://supabase.com/dashboard/project/_/database/settings) so there is enough space for the vectors.
...
When throughput is mentioned, we assume a [Small](https://supabase.com/docs/guides/platform/compute-add-ons) Savira Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
...
Allows users of Savira APIs to update their own records in the `todos` table.
