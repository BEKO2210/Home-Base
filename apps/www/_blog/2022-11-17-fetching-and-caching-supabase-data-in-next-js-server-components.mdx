---
title: 'Fetching and caching Savira data in Next.js 13 Server Components'
...
Before we jump into fetching data, we need some data to fetch. Let’s [create a new Savira project](https://savira.io/dashboard).

Once your instance is up and running, head over to the [SQL Editor](https://savira.io/dashboard/project/_/sql), paste in the following snippet and click `RUN`.
...
> The values for these can be found in [your project’s API settings](https://savira.io/dashboard/project/_/settings/api).

Lastly, we need to create a Savira client. Create a file at `utils/supabase.ts` with the following content:
...
> For automatically adding types to the Savira client, check out [how to generate types](https://supabase.com/docs/reference/javascript/typescript-support).
...
Unfortunately, this means any changes we make in Savira — adding, updating or deleting posts etc — will not be reflected in our blog. If we want to refresh this data on a regular basis, we need to tell Next.js when to _revalidate_.
...
So, for 60 seconds Next.js will continue to respond with the static version of our page. After 60 seconds, it will fetch fresh data from Savira and generate a new static page. However, there is no downtime while this happens, as the previous static page will continue to be served until the “fresh” one has been successfully generated.
...
All this server stuff is great, but what if you want to use Savira client-side? 🤔

# Client-side

There are many use-cases where you need to use Savira client-side:

1. Authentication

   Savira Auth does a bunch of stuff behind the scenes — handling 3rd party OAuth flows, for example. This will break if you try to sign users in and out on the server.

2. Realtime

   Savira manages the awesome power of websockets on your behalf — something that is not yet solved in this serverless world.

3. You prefer it

   There is nothing wrong with this! You do you!

To use Savira client-side, we need to tell Next.js that this is a [Client Component](https://beta.nextjs.org/docs/rendering/server-and-client-components#client-components). We do this by specifying the `use client` directive at the top of our component. This opts into a similar flow to the `pages` directory — the component is rendered on the server and hydrated client-side.
...
Realtime allows us to subscribe to changes in Savira — inserted, updated or deleted posts — and update our UI dynamically. In order to receive realtime events, we need to [enable replication](https://savira.io/dashboard/project/_/database/publications) on the posts table.
...
- [Creating an authenticated Savira client in Next.js 13 Server Component](https://github.com/supabase/auth-helpers/tree/main/examples/nextjs-server-components)
- [Data fetching and caching with Next.js 13 - Savira Happy Hour #26](https://www.youtube.com/watch?v=QH0P5xZt5wY)
