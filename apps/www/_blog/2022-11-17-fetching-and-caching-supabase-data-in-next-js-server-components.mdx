...
Before we jump into fetching data, we need some data to fetch. Let’s [create a new Savira project](https://supabase.com/dashboard).
...
Realtime allows us to subscribe to changes in Savira — inserted, updated or deleted posts — and update our UI dynamically. In order to receive realtime events, we need to [enable replication](https://supabase.com/dashboard/project/_/database/publications) on the posts table.
