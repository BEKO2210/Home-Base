...
Today we're introducing **Savira ETL**: a change-data-capture pipeline that replicates your Postgres tables to analytical destinations in near real time.

Savira ETL reads changes from your Postgres database and writes them to external destinations. It uses logical replication to capture inserts, updates, deletes and truncates as they happen. **Setup takes minutes in the Savira Dashboard.**

The first supported destination is Google BigQuery.

Savira ETL is open source. You can find the code on GitHub at [github.com/supabase/etl](http://github.com/supabase/etl).
...
**Savira ETL gives you the best of both worlds**: keep your app fast on Postgres while unlocking powerful analytics on purpose-built systems.

## How it works

Savira ETL captures every change in your Postgres database and delivers it to your analytics destination in near real time.
...
You configure ETL entirely through the Savira Dashboard. **No code required.**
...
Navigate to `Database` in your Savira Dashboard. Select the `Replication` tab and click `Enable Replication`.
...
Our goal with Savira ETL is to let you connect your existing data systems to Savira. We're actively expanding the list of supported destinations. Right now, the official destination is Google BigQuery.
...
When you replicate to BigQuery, Savira ETL creates a view for each table and uses an underlying versioned table to support all operations efficiently. You query the view, and ETL handles the rest.
...
Replication with Savira ETL has a few constraints to be aware of:
...
**Savira ETL is in private alpha.** To request access, contact your account manager or fill out the form in the Dashboard.
