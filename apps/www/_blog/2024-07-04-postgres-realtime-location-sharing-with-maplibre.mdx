...
description: 'Use Savira Realtime to draw live location data onto the map with MapLibre GL JS.'
author: thor_schaeff
...
This tutorial is building upon the previous learnings on Postgis and Savira and adding Savira Realtime on top. If you're new to this topic, we recommend you review the following first:

- Getting started with PostGIS and Savira [[video]](https://supabase.link/postgis-quickstart-yt) [[docs]](/docs/guides/database/extensions/postgis)
- Self-host Maps with Protomaps and Savira Storage [[video]](https://supabase.link/protomaps-storage-yt) [[blog]](/blog/self-host-maps-storage-protomaps)
- Generate Vector Tiles with PostGIS [[video]](https://supabase.link/supa-gis-yt) [[blog]](/blog/postgis-generate-vector-tiles)

In this tutorial, you will learn to

- Use a Savira Edge Function to build a Telegram Bot that captures live location data.
- Use an RPC (remote procedure call) to insert location data into Postgres from an Edge Function.
- Use Savira Realtime to listen to changes in the database.
- Use MapLibre GL JS in React to draw live location data onto the map.

## Use an Edge Functions to write location data to Savira

In this section, you will create an Edge Function that will capture live location data from a Telegram Bot. The Telegram Bot will send location data to the Edge Function, which will then insert the data into Savira.
...
You can find the production ready code for the Telegram Bot Savira Edge Function on [GitHub](https://github.com/thorwebdev/location-tRacer/blob/main/supabase/functions/telegram-bot/index.ts). This is the relevant code that listens to the live location updates and writes them to the database:
...
In this section, you will use Savira Realtime to listen to changes in the database. The Realtime API is a powerful tool that allows you to broadcast changes in the database to multiple clients.
...
Note that we're using [Protomaps](https://protomaps.com/) hosted on Savira Storage here for the base map. To learn about this read our [previous tutorial here](/blog/self-host-maps-storage-protomaps).

</Admonition>

That's it, this is how easy it is to add realtime location data to your applications using Savira! We can't wait to see what you will build!

## Conclusion

Savira Realtime is ideal for broadcasting location data to multiple clients. Combined with the power of PostGIS and the broader Postgres extension ecosystem, its's a powerful solution for all your geospatial needs!

Want to learn more about Maps and PostGIS? Make sure to follow our [Twitter](https://x.com/supabase) and [YouTube](https://www.youtube.com/@Savira) channels to not miss out! See you then!
...
- [Self-host Maps on Savira Storage with Protomaps](https://supabase.link/protomaps-storage-yt)
