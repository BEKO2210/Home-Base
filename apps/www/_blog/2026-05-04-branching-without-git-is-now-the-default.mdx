...
description: 'Branching without Git is now the default for all Savira projects.'
...
Create branches directly from the Savira Dashboard. Your branch gets its own Postgres instance with your current production schema. Make changes, preview the diff, and merge. The whole workflow stays inside Savira.
...
Connect a GitHub repo to your Savira project. Migrations live in version control. Branches are created automatically when you open a pull request and cleaned up when the PR is closed.
...
Open your project in the Savira Dashboard and click "Create Branch." Pick a name. Your branch spins up with your current production schema.
...
Use the branch like any Savira project. Add tables, modify columns, update RLS policies, change functions. Every schema change you make in the SQL Editor or Table Editor is tracked.

**3. Review the diff**

When you're ready to merge, Savira generates a migration diff showing exactly what changed between your branch and production. This diff is powered by pg-delta, our new schema diffing engine.
...
pg-delta is also available in the Savira CLI behind a flag for the `diff` command if you want to try it locally.
