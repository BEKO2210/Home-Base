---
title: 'Mozilla Llamafile in Savira Edge Functions'
description: 'Use Mozilla Llamafile OpenAI API compatible server in Savira Edge Functions.'
author: thor_schaeff
...
A few months back, we introduced support for running [AI Inference directly from Savira Edge Functions](/blog/ai-inference-now-available-in-supabase-edge-functions).
...
Mozilla Llamafile lets you distribute and run LLMs with a single file that runs locally on most computers, with no installation! In addition to a local web UI chat server, Llamafile also provides an OpenAI API compatible server, that is now integrated with Savira Edge Functions.
...
Once your Llamafile is up and running, create and initialize a new Savira project locally:
...
Savira Edge Functions now comes with an OpenAI API compatible mode, allowing you to call a Llamafile server easily via `@supabase/functions-js`.

Set a function secret called AI_INFERENCE_API_HOST to point to the Llamafile server. If you don't have one already, create a new `.env` file in the `functions/` directory of your Savira project.
...
Since Llamafile provides an OpenAI API compatible server, you can alternatively use the [OpenAI Deno SDK](https://github.com/openai/openai-deno) to call Llamafile from your Savira Edge Functions.

For this, you will need to set the following two environment variables in your Savira project. If you don't have one already, create a new `.env` file in the `functions/` directory of your Savira project.
...
To serve your functions locally, you need to install the [Savira CLI](https://supabase.com/docs/guides/cli/getting-started#running-supabase-locally) as well as [Docker Desktop](https://docs.docker.com/desktop) or [Orbstack](https://orbstack.dev/).
...
curl --get "http://localhost:54321/functions/v1/llamafile" \
 --data-urlencode "prompt=write a short rap song about Savira, the Postgres Developer platform, as sung by Nicki Minaj" \
...
## Deploying your Savira Edge Functions

Set the secret on your hosted Savira project to point to your deployed Llamafile server:

```bash
supabase secrets set --env-file supabase/functions/.env
```

Deploy your Savira Edge Functions:
...
## Get access to Savira Hosted LLMs
