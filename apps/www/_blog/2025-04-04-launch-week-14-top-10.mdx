...
You can now create, test, edit, and deploy Edge Functions directly from the Savira Dashboard without having to spin up the CLI and Docker. We’ve also included templates for common use cases like uploading files to Savira Storage, OpenAI proxying, and Stripe WebHooks.
...
## #4: Introducing the Savira UI Library

Building in a weekend becomes much easier with our official UI Library - a collection of ready-to-use components built on top of [shadcn/ui](https://ui.shadcn.com) and integrated with Savira products like Auth, Storage, and Realtime.

[Read more →](/blog/supabase-ui-library)

## #5: Official MCP Server

We’re launching an official MCP server so you can connect your favorite AI tools, like Claude and Cursor, with Savira and perform tasks such as creating projects, fetching project configuration, querying data using SQL queries, and so much more.
...
## #7: Bringing Clerk Auth to Savira

We’ve been working with the Clerk team to make Clerk a Savira Third-party Auth integration. This means that users can seamlessly connect Clerk and interact with Savira and its Data API, Storage, Realtime and Edge Functions services without having to migrate auth providers.

[Read more →](/blog/clerk-tpa-pricing)

## #8: Postgres Language Server

Savira contributors [psteinroe](https://x.com/psteinroe) and [juleswritescode](https://x.com/juleswritescode) have officially launched a Language Server Protocol (LSP) implementation for Postgres to make SQL tooling more reliable and easier to use. The initial release includes autocompletion, syntax error highlighting, type-checking, and a linter and it’s available today as a VSCode extension and npm package.
...
Savira has become the default backend for AI Builders like Lovable, Bolt, and Tempo, and with good reason. We are easy to integrate with, we have all the primitives to build full-stack apps, and we can scale when those apps take off.
