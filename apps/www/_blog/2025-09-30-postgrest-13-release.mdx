...
You can check the `Proxy-Status` and `Content-Length` headers in the Savira Logs Explorer.
...
If you use Savira Auth or the CLI to create JSON Web Keys, you shouldn’t worry about this change as both systems will ensure `kid`'s are present in the JSON Web Key Set.
...
PostgREST v13 is now available for all new projects on the Savira platform, old projects can upgrade to get this new version.
