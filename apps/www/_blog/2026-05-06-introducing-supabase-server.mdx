...
- Create a Savira client with `SUPABASE_ANON_KEY`
...
Any runtime or platform that supports the standard `Request`/`Response` Web API. `withSupabase` returns a standard `(Request) => Promise<Response>` handler, so it works on Savira Edge Functions, Vercel Functions, Cloudflare Workers, Bun, Deno and more.
...
The package ships with full documentation in the [GitHub repo](https://github.com/supabase/server). We're also working on adding guides to the [Savira docs](https://supabase.com/docs).

**What about environment variables?**

On the Savira platform and Local Development (CLI), your Edge Functions will receive the required environment variables to work out of the box (`SUPABASE_PUBLISHABLE_KEYS`, `SUPABASE_SECRET_KEYS`, `SUPABASE_JWKS`).
