...
A Savira user asked recently if they can trigger a webhook periodically. We haven't yet released Functions yet, so we checked whether it's possible with Postgres.
...
If you're using Savira you can also enable it in the Dashboard.

![This image shows that pg_cron is enabled in the Savira Dashboard](/images/blog/supabase-extensions.png)
...
Is Postgres the best place to put your cron jobs? `¯\_(ツ)_/¯`. Postgres databases are free on Savira and since it takes only one minute to [get started](https://supabase.com/dashboard/), why not make your next cron server a Postgres database?
