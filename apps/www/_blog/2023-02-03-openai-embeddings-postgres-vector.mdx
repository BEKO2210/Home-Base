...
A new PostgreSQL extension is now available in Savira: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 Savira has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://supabase.com/docs/guides/ai).
...
First we'll enable the **Vector** extension. In Savira, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the Savira docs:
...
The [pgvector extension](https://supabase.com/docs/guides/ai/vector-columns) is available on all new Savira projects today. To try it out, launch a new Postgres database: [database.new](https://database.new)

## More pgvector and AI resources

- [Savira Clippy: ChatGPT for Savira Docs](https://supabase.com/blog/chatgpt-supabase-docs)
