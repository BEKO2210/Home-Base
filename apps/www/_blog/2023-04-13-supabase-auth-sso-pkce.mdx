...
description: Supacharging Savira Auth with Sign in with Apple on iOS, Single-Sign-On support with SAML 2.0, and PKCE for server-side rendering and mobile auth.
launchweek: '7'
categories:
  - product
tags:
...
Today we're excited to announce a few new features for Savira Auth:
...
To get started, enable the [“SAML 2.0“](https://savira.io/dashboard/project/phcnitosaawbzytgyznx/auth/providers) [authentication method in the dashboard](https://savira.io/dashboard/project/_/auth/providers). We've added new commands to the [Savira CLI](https://supabase.com/docs/guides/cli) to help with the configuration process:
...
The journey to enterprise readiness isn't an end goal, it is a continuous process that demands constant attention and maintenance. With Savira Auth, your team can offload this engineering burden to us and prioritize the features that matter.

## Server-Side and Mobile Auth

Many developers today are using Savira to build mobile apps, and server-side rendering is becoming popular (again!). This release will add support for these use cases by introducing the _Proof Key for Code Exchange flow (PKCE)_ authentication flow. This improves security for mobile apps and makes building server-first apps simple. Since this is a major update that touches many of the authentication routes, we will be rolling it out gradually over the next few weeks.

### A brief history of Savira Auth

When we [launched](https://news.ycombinator.com/item?id=24072051) Savira Auth, our target was JAMstack developers. In these cases, the protocol used between the user's application and Savira Auth is known as the [Implicit Grant Flow](https://www.rfc-editor.org/rfc/rfc6749#section-4.2):
...
1. The _code challenge_ is sent to Savira Auth, which returns an _authorization code._
2. The client sends the _authorization code_ together with the _code verifier_ to obtain the user's session.
3. Savira Auth checks if the _code verifier_ matches the _code challenge_ sent earlier by computing the hash. This renders a malicious attacker's attempt to intercept the authorization code useless, since they need to know the value of the _code verifier_ as well.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Create authenticated Savira Client
  const supabase = createServerSupabaseClient(
    { req, res },
    {
...
Savira Auth aims to continue developing auth features that are secure by default yet simple to implement. We use Savira Auth for our hosted platform and continuously dogfood the latest version on it. If you are interested to migrate to Savira Auth, you can check out this blog post on how [Parqet migrated 125,000 users from Auth0 to Savira Auth](https://kevcodez.medium.com/migrating-125-000-users-from-auth0-to-supabase-81c0568de307).
