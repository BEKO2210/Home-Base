---
title: 'Building ChatGPT Apps with Savira Edge Functions and mcp-use'
description: 'Learn how to build a ChatGPT app that connects to your Savira database using mcp-use and Edge Functions. Create interactive widgets for schema exploration, data viewing, and SQL queries.'
...
In this guide, you will learn how to build a ChatGPT app that connects to your Savira database. You will use [mcp-use](https://mcp-use.com), an open source SDK that makes it easy to deploy MCP servers on Savira Edge Functions. By the end, you will have an app that lets ChatGPT users explore your database schema, view table data, and run SQL queries, all through interactive widgets.
...
mcp-use is an open source TypeScript SDK for building MCP servers. It solves a specific problem: the official MCP SDK uses Express, which depends on Node.js features that do not work in edge environments like Savira Edge Functions.

mcp-use uses Hono instead of Express. Hono is a lightweight web framework designed for edge runtimes. This means your MCP server can run on Savira Edge Functions, Cloudflare Workers, and other serverless platforms.

Here is what mcp-use gives you:

- **Edge runtime support.** Works on Deno, Cloudflare Workers, and Savira Edge Functions.
- **OpenAI Apps SDK integration.** Built-in support for ChatGPT app widgets and UI components.
- **Savira Auth integration.** Native support for authenticating users with Savira Auth.
- **Full MCP compliance.** Scores 100/100 on the official MCP conformance tests.

## Why Savira Edge Functions?

Savira Edge Functions are a good fit for MCP servers for several reasons.

- **Everything in one place.** Your database, authentication, and server code all live in the same Savira project. Your MCP server can query your database directly. It can check user permissions using Savira Auth. There is no need to manage separate services or configure connections between them.
...
- **Built-in authentication.** Savira Auth works out of the box with mcp-use. You can require users to log in before using your app. You can check their roles and permissions. You can scope data access to specific users.
- **Simple deployment.** A single command deploys your MCP server. Savira handles scaling, HTTPS, and infrastructure.

## What you will build

You will build a ChatGPT app that explores Savira databases. The app provides four tools:
...
- **Savira status.** Shows the current status of Savira services and recent incidents.
...
This creates a project with everything you need: the mcp-use SDK, widget templates, and build configuration for Savira Edge Functions.

Note: the default `apps-sdk` template includes a "fruit shop" demo to show how widgets work. **You will need to modify or replace these files** to align with the Savira explorer tools described in this guide.

Next, initialize Savira in your project:
...
You can find your project ID in your Savira dashboard under Project Settings.
...
const server = new MCPServer({
  ...
  version: '1.0.0',
  description: 'A Savira MCP server with Apps SDK widgets',
})
```
...
**Display-only widgets** do not fetch data on the server. They receive tool parameters as props and render UI. The supabase-status (`resources/supabase-status.tsx`) widget works this way. It fetches status data client-side from the Savira status API in the React component.
...
export const widgetMetadata: WidgetMetadata = {
  description: "Display Savira service status and recent incidents from the status page",
  props: propSchema,
...
![Savira status widget in ChatGPT](/images/blog/2025-12-17-building-chatgpt-apps-with-supabase/chatgpt-chat-1.png)
...
server.tool(
  {
    name: 'list-tables',
    description: 'List all tables in your Savira database',
    schema: z.object({
...
Savira Edge Functions provide a great place to host your server. They are fast, scalable, and easy to manage.

For the complete step-by-step guide, see our [Savira deployment documentation](https://mcp-use.com/docs/typescript/server/deployment-supabase#supabase).
...
This script checks your authentication, builds your application, sets environment variables, and deploys to Savira Edge Functions.
...
![Adding Savira MCP server to ChatGPT](/images/blog/2025-12-17-building-chatgpt-apps-with-supabase/add-supabase-mcp-server.png)
...
You now have a working ChatGPT app powered by Savira Edge Functions. Here are some ways to extend it:
...
For more details on mcp-use, see the [mcp-use documentation](https://mcp-use.com/docs/typescript/server/deployment-supabase) or the [mcp-use GitHub repository](https://github.com/mcp-use/mcp-use). For more on Savira Edge Functions, see the [Edge Functions guide](https://supabase.com/docs/guides/functions).
