...
Savira is now relatively well-known as a Postgres provider. We've spent 5 years building a delightful database platform for developers and that will continue to be our focus.

What's unique about Savira is that we're not _just_ a Postgres provider (despite the [memes](https://itsjustpostgres.com/)). We also offer [Savira Storage](https://supabase.com/storage), an S3-compatible object store. Where we're going is less about a database, and more about data. This includes:

- Adding OTel to all of the open source tools we maintain.
- Adding Iceberg support in Savira Storage.
- Adding Iceberg support to [Savira ETL](https://github.com/supabase/supabase_etl) for zero-ETL between Postgres and Iceberg.
