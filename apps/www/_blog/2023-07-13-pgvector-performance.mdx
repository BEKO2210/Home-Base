...
All compute add-ons available on Savira were used to run our benchmarks. Each add-on variant has a different allocation of RAM and CPU cores, the details of which are available in our docs. Each Savira compute add-on comes with a specific set of optimizations ([version 2023-07](https://gist.github.com/egor-romanov/323e2847851bbd758081511785573c08)).
...
- We used the same hardware: a 2XL instance on Savira, which offers the same core and RAM count as NirantK's machine - 8 cores and 32 GB of RAM
...
1. Over-provision RAM during preparation. You can scale down in step `5`, but it's better to start with a larger size to get the best results for RAM requirements. (We'd recommend at least 8XL if you're using Savira.)
...
pgvector is still early in development. As with any open source tool, it needs time and resources to make it better. Savira plans to continue supporting Andrew with his development of pgvector.
