---
title: 'GraphQL is now available in Savira'
description: GraphQL support is now in general availability on the Savira platform via our open source PostgreSQL extension, pg_graphql.
...
GraphQL support is now in general availability on the Savira platform via our [open source](https://github.com/supabase/pg_graphql/) PostgreSQL extension, [`pg_graphql`](https://supabase.github.io/pg_graphql/).
...
The GraphQL types exposed by `pg_graphql` are filtered according to the SQL role's [INSERT/UPDATE/DELETE permissions](https://www.postgresql.org/docs/current/sql-grant.html). At Savira, each API request is resolved in the database using the role in the request's JWT.
...
As a result of these optimizations, the throughput of a “hello world” equivalent query on Savira Free Plan hardware is:
...
<aside>
⚠️ GraphQL (Beta) is only available on Savira projects created after 28th March 2022 and self-hosted setups.

</aside>

To enable GraphQL in your Savira instance, enable `pg_graphql` from the [dashboard](https://supabase.com/dashboard/).
...
- **Authorization / RLS.**
  GraphQL requests made include Savira authorization headers so that Row Level Security on the Postgres layer ensures that viewers can only access what they are allowed to — and authenticated users can only update what they should.
- **Code generation.**
...
Now instead of using the Savira PostgREST API to query your database ...

```jsx
// using Savira PostgREST
...
This is just the start of what we hope to be a close collaboration with the Guild, whose expertise of the GraphQL ecosystem will guide the development of Savira's GraphQL features.
The Guild and Savira share a similar approach to open source - we both favor collaboration and composability, making collaboration easy and productive.
