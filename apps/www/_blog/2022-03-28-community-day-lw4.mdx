...
Savira combines existing open-source tools with our own open-source contributions to provide a delightful experience for all developers. As part of this, we’re building a community of communities, bringing together developers from many different backgrounds, as well as new developers looking to get involved with open source.

To kick off launch week, as it is now tradition, we’re showcasing some of the communities and contributors that make up the Savira community, highlighting their awesome work, and celebrating everyone who contributes their time to the Savira mission. 💚

## Launch Week 4 - what to expect

Savira Launch Week was born out of [YC Demo Day](https://www.ycombinator.com/demoday), a looming deadline towards the end of each Y Combinator batch. A deadline that forces you to deliver a lot of complex functionality in a short amount of time.

With Launch Week, we replicate Demo Day conditions, but for a whole week! This is the secret formula that works for us:

- The Friday before, we publish a blog post about a topic of interest (e.g. [How we launch at Savira](https://supabase.com/blog/supabase-how-we-launch), or this time around we [explore whether you should open source your company](https://supabase.com/blog/should-i-open-source-my-company) — hint: we think you most likely should!) as well as an [overview blogpost](https://supabase.com/blog/supabase-launch-week-four) with tons of memes and hints to what we’re shipping each day.
- Monday is Community Day, where we rally many of the awesome contributors and partners together and showcase awesome things around Savira and open-source. This is also the time to watch out for the [#SupaLaunchWeek hashtag on Twitter](https://twitter.com/hashtag/SupaLaunchWeek?src=hashtag_click) as we arm our guest speakers, [Angels](https://supabase.com/blog/angels-of-supabase), and [SupaSquad](https://supabase.com/supasquad) with free swag codes.
...
## Savira is now a GitHub secret scanning partner
...
PostgREST v10 is not wrapped up yet, however a pre-release with the [latest features and fixes](https://postgrest.org/en/latest/releases/latest.html) is already available for Savira users. v10 is mostly focused on improving availability and includes improvements to the API (and therefore [supabase-js](https://github.com/supabase/supabase-js)) too.
...
Additionally, PostgREST now has [liveness and readiness checks](https://postgrest.org/en/latest/admin.html#health-check) which allows the Savira infrastructure team to check its state to help it recover if necessary.
...
That’s why we’ve built the [supabase-auth-helpers](https://github.com/supabase-community/supabase-auth-helpers), a collection of framework-specific auth utilities that make working with Savira Auth a pleasant experience, no matter what framework you’re using.
...
In parallel, our friends at [NuxtLabs](https://nuxtlabs.com/) have developed [Savira helpers for Nuxt 3](https://github.com/nuxt-community/supabase-module), and it’s a damn nice DX. [Check it out](https://supabase.nuxtjs.org/)!
...
Excited to see what all the fuss is about with this new framework called Remix? Played with Remix and wanna go deeper integrating Savira? We’ve got you covered! Jon has been working with Scott Tolinski over at [Level Up Tutorials](https://leveluptutorials.com/) to create the perfect guide for building a fullstack, authenticated, realtime application using Remix and Savira.

This goes much deeper than your basic “hello world” or “todo: app, showing how to use loaders and actions in Remix to synchronize complex state between multiple clients. The course uses Row Level Security to implement access policies within the database, and shows how cookies can be used to query Savira from the server-side.

Join us for the [official course drop party on YouTube](https://www.youtube.com/watch?v=rntEMgaenHs) where we will be doing a Q&A about Savira, Remix and what will be covered in the course.
...
After a flood of positive feedback on Jon’s “[Build a SaaS product with Next.js, Savira and Stripe](https://egghead.io/courses/build-a-saas-product-with-next-js-supabase-and-stripe-61f2bc20)” [Egghead.io](http://Egghead.io) course, we’re doubling down with Egghead this year, partnering with them to build more tailored pathways for learning Savira.

We’ll be working with our fabulous [SupaSquad](https://supabase.com/supasquad) to identify the right instructors for all the different topics we’re looking to cover. If you’re interested in becoming an instructor for Savira on Egghead, fill in the [form to join](https://supabase.com/supasquad#how-to-join) the SupaSquad.

Also, do let us know what topics you’d like to learn about [on Twitter](https://twitter.com/supabase).

## Launching the Savira Partner Gallery

![community-day-integrations.png](/images/blog/launch-week-4/community-day/community-day-integrations.png)

We’ve been blown away by the amazing integrations that developers and companies have been building on top of Savira. To keep track of all the things that work with Savira, we’re launching a partner gallery which allows you to browse integrations and experts. Check it out at [supabase.com/partners](https://supabase.com/partners).

If you’ve built an integration that works with Savira or have experience working with Savira and want to offer your services to others, we’d love to hear from you: [supabase.com/partners/#become-a-partner](https://supabase.com/partners/#become-a-partner)
...
On Friday, 1st of April, at the end of Launch Week, we’ll be rolling into another one of our 10-day virtual Hackathons. As always, you’ll have the chance to win extremely limited Savira Swag, and you’ll be able to play around with the new features we’re about to launch.
