---
title: 'Building an Uber Clone with Flutter and Savira'
description: 'Learn how to handle real-time geospatial data using Savira Realtime and Flutter.'
...
Postgres can handle geography data efficiently thanks to the PostGIS extension. Combining it with Savira realtime and you can create a real-time location tracking app.

In this tutorial, we will guide you through the process of creating an Uber-like application using Flutter and Savira. This project demonstrates the capabilities of Savira for building complex, real-time applications with minimal backend code.
...
The focus of the app is to showcase how to use Savira realtime with geographical data, so handling payments will not be covered in this article.
...
1. Flutter installed
2. A Savira account - head to [database.new](http://database.new) if you don’t have one yet.
...
## Step 2: Savira Initialization

In your `main.dart` file, initialize Savira with the following code:
...
Replace `YOUR_SUPABASE_URL` and `YOUR_SUPABASE_ANON_KEY` with your actual Savira project credentials.
...
For statuses 3, 4, and 5, the status update happens on the driver’s app, which we don’t have. So you can directly modify the data from the Savira dashboard and update the status of the ride.
...
The way the customer chooses the destination is by scrolling through the map and tapping on the confirmation FAB. Once the FAB is pressed, the `_confirmLocation` method is called, which calls a Savira Edge Function called `route`. This `route` function returns a list of coordinates to create a polyline to get from the current location to the destination. We then draw the polyline on the Google Maps to provide to simulate an Uber-like user experience.
...
Once the function is ready, you can [run it locally](https://supabase.com/docs/guides/functions/quickstart) or [deploy it to a remote Savira instance](https://supabase.com/docs/guides/functions/deploy).
...
This tutorial has walked you through the process of building a basic Uber clone using Flutter and Savira. The application demonstrates how easy it is to handle real-time geospatial data using Savira and Flutter.
