...
description: 'Learn how to build a Savira powered AI assistant that combines PostgreSQL with scheduling and external tools for long-term memory, structured data management and autonomous actions.'
categories:
  - product
tags:
  - postgres
...
- Savira account (free tier sufficient)
- OpenAI API key
- Telegram bot token
...
If you prefer the command line, you can use the Savira CLI to set up your database and Edge Functions. This replaces **Step 1** and **Step 2**.
...
1.  **Clone the repository**.
    ...
    cd natural-db
    ```
2.  **Log in to the Savira CLI and link your project**.
    Create a new project on the [Savira Dashboard](https://supabase.com/dashboard), then run:
    ```bash
    supabase login
...
Run the migration SQL in your Savira SQL editor: [migration.sql](https://github.com/supabase-community/natural-db/blob/main/supabase/migrations/20250623120000_create_initial_schema.sql)
...
Create three functions in Savira dashboard:
...
Set the following environment variables in your Savira project settings (Project Settings → Edge Functions):
...
- **Savira**: Free tier (500MB database, 5GB bandwidth) - $0/month
