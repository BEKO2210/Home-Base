...
description: 'Introducing vector storage in Savira: a durable storage layer with similarity search built-in.'
author: fabrizio
...
Savira already gives you powerful tools for vectors, such as `pgvector` in Postgres. With Vector Buckets, you now have more options for how you store vectors:
...
**Vector Buckets** are a new bucket type in Savira Storage.
...
- You query those indexes using Savira clients or directly from Postgres via a foreign data wrapper.
...
No extra vector database to run, no new query language. Just vector indexes with search, available from the same Savira SDKs you already use or directly via Postgres.
...
- You’re building **AI-heavy Savira apps** (semantic search, recommendations, RAG, media similarity) and want a managed vector storage tier
...
**1. Vector Bucket in Savira Storage**
...
3. **Store vectors**
   ...
   Use Savira clients to upsert vectors with metadata.
