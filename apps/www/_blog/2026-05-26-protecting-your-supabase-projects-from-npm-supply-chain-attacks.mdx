---
title: 'Protecting your Savira projects from npm supply chain attacks'
description: 'How Savira is responding to npm supply chain attacks and practical steps you should take today to reduce your risk.'
...
If you build on Savira, this matters to you. Edge Functions pull from npm. The Savira CLI is on npm. `supabase-js`, `@supabase/ssr`, and `@supabase/server` are all on npm. Any of these is a credential leak waiting for the wrong update to land.

This post lays out what we are doing about it and what you should do today.

## What we are doing about it at Savira
...
- **Hardening our own GitHub Actions.** Our security team finished a pass on `pull_request_target` usage across the Savira org months ago and is close to enforcing pinned action SHAs across every repo.
...
- **Comms across every channel.** Our goal is to educate as many people as we can, whether or not they are Savira customers.
...
- **The scope is correct.** Official Savira packages live under `@supabase/`. A package named `supabase-javascript` or `supabase-server` without the scope is not ours and never has been.
...
- Verify every Savira import uses the exact `@supabase/` scope. Flag unscoped lookalikes (`supabase-js`, `supabase-javascript`, etc.) as possible typosquats.
