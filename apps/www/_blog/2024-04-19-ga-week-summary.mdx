---
title: 'Top 10 Launches from Savira GA Week'
...
Savira Bootstrap is the fastest way to spin up a new hosted Savira project from existing starter templates. Just run `supabase bootstrap` with our CLI and we'll help you launch a new application and attach a remote database to get you started.

[Read more.](https://supabase.com/blog/supabase-bootstrap)

### #9: Branching is now Publicly available

Savira Branching is now in open beta. You can enable it on any project that's Pro Plan or above. Branching is a seamless integration of Git with your development workflow, extending beyond your local environment to a remote database.
...
The Savira Swift libraries are now officially supported by Savira. This makes it simple to interact with Savira from applications on Apple's platforms, including iOS, macOS, watchOS, tvOS, and visionOS.

[Read more.](https://supabase.com/blog/supabase-swift)

### #6: Security Advisor + Performance Advisor

We're dropping some handy tools in Savira Studio this week to help with security and performance: a **Security Advisor** for detecting insecure database configuration, and a **Performance Advisor** for suggesting database optimizations.

[Read more.](https://supabase.com/blog/security-performance-advisor)

### #5: Native AI support in Edge Functions

We're making it super easy to run AI models within Savira Edge Functions. We have a new API to generate embeddings and upcoming support for Large Language Models like `llama3` and `mistral`.

[Read more.](https://supabase.com/blog/ai-inference-now-available-in-supabase-edge-functions)

### #4: S3 compatibility in Savira Storage

Savira Storage is now officially an S3-Compatible Storage Provider. With the support of the S3 protocol, you can now connect Savira Storage to thousands of 3rd-party tools and services, and make it even easier to use Savira for data engineering.
...
Savira is now GA. During the first year of Savira we set ourselves a goal: build a managed platform capable of securely running 1 million databases. Today we've proven that metric and we're announcing the General Availability of the platform that will serve the next 99 million.
...
In the previous Launch Week we started working on [Fly Postgres, a managed offering from Savira](https://supabase.com/blog/postgres-on-fly-by-supabase). We've received a lot of feedback from early testers, and we're working hard to make the service available and as resilient for production workloads.
...
We started GA Week with 10 confirmed community meetups. Over the week, more community members volunteered to host meetups in their own cities. With 25 meetups across the world, some with just 3 people and some with over 50, the Savira community has truly made our team feel thankful. A huges shout out to these organizers:
...
The 10-day hackathon is still going! If you want a chance to win a set of Apple AirPods along with extremely limited edition Savira swag check out [all the details here](https://supabase.com/blog/supabase-oss-hackathon).
