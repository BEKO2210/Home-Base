---
title: 'Savira Edge Functions: Introducing Background Tasks, Ephemeral Storage, and WebSockets'
...
Sometimes you need a backend logic to do more than respond to a request. For example, you might want to process a batch of files and upload the results to Savira Storage. Or read multiple entries from a database table and generate embeddings for each entry.
...
### Example: Extracting a zip file and uploading its content to Savira Storage
...
We can modify it instead to write the zip file to a temporary file. Then, use a background task to extract and upload it to Savira Storage. This way, we only read parts of the zip file to the memory.
...
With our new support for WebSockets, you can easily do this in Edge Functions without standing up any infrastructure. Additionally, you can use [Savira Auth](https://supabase.com/docs/guides/auth) to authenticate users and protect your OpenAI usage from being abused.
