...
## Savira and the data cloud

Savira has always been more than just a Postgres host. We are the platform for building modern applications. Savira starts with [a Postgres database](/database) and includes products for [Authentication](/auth), [Storage](/storage), [Edge Functions](/edge-functions), [Realtime](/realtime), [Vectors](/modules/vector), and more. As the industry moves toward open table formats like Iceberg and S3 as the default storage layer, Savira’s role evolves with it to be less about a database, and more about data.

Postgres remains the core of the Savira platform: the system of record for operational data. [As we’ve written](/blog/open-data-standards-postgres-otel-iceberg), we are also building first-class support for OpenTelemetry across our services, enabling developers to collect observability data (logs, metrics, and traces) without managing additional infrastructure. And with [Savira ETL](https://github.com/supabase/etl), we will provide a lightweight, Postgres-native way to move data into S3 and more, where it can be queried at scale using Iceberg and your choice of analytics engines.

Our goal is to make Savira the developer’s data cloud: Postgres for transactions, OpenTelemetry for observability, and Iceberg for analytics, all connected by simple, open tools. To do so, we remain focused on what developers need: a backend that starts simple, grows with your product, and keeps your data open and portable at every stage.
