...
Today we’re releasing [automatic embeddings](/docs/guides/ai/automatic-embeddings) - automate embedding generation and updates using Savira [Vector](/modules/vector), [Queues](/modules/queues), [Cron](/modules/cron), and [pg_net](/docs/guides/database/extensions/pg_net) extension, and [Edge Functions](/edge-functions).
...
This is the pattern we use at Savira. It uses a few common extensions:
...
You can inspect the queue, retry failed jobs, and customize the Edge Function used to generate embeddings. You can find the complete reference implementation in the [Savira Automatic Embeddings Guide](/docs/guides/ai/automatic-embeddings).
...
This will kick off the embedding pipeline within a Savira Edge Function. If you were to immediately query for the document you just inserted, the `embedding` column will be empty:
...
- [Get started with Savira](/dashboard/sign-in?returnTo=%2Fprojects) and try it out today
