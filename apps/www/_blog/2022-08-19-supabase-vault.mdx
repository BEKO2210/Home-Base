...
Today we're announcing [Savira Vault](https://supabase.com/docs/guides/database/vault), a Postgres extension for managing secrets and encryption inside your database.
...
Vault is now available on every Savira project. [Check it out](https://supabase.com/dashboard/project/_/settings/vault/secrets)
...
Like most cloud providers, Savira offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
Savira Vault allows you to store secrets without leaking any sensitive information.
...
The core of the Savira Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. Savira generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
At Savira, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
