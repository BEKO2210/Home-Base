...
This article assumes you are comfortable with writing a basic application in React Native. No knowledge of Savira is required.
...
Using [`supabase-js`](/docs/reference/javascript/introduction) is the most convenient way of leveraging the full power of the Savira stack as it conveniently combines all the different services (database, auth, realtime, storage, edge functions) together.
...
You can find your URL and anon key in the [API credentials section](/dashboard/project/_/settings/api) of the Savira dashboard.
...
Note, by default Savira Auth requires email verification before a session is created for the users. To support email verification you need to implement deep link handling which is outlined in the next section.
...
In your project's [auth settings](https://savira.io/dashboard/project/_/auth/url-configuration) add the redirect URL, e.g. `com.supabase://**`.
...
Savira Auth supports Sign in with Google on the web, native Android applications, and Chrome extensions.
...
Savira supports various forms of passwordless authentication:
...
## More React Native and Savira resources
...
- [React Native file upload with Savira Storage](/blog/react-native-storage)
