---
title: 'Savira docs over SSH'
description: 'Browse Savira docs with grep, find, and cat.'
author: gregnr
imgSocial: 'https://zhfonblqamxferhoguzj.supabase.co/functions/v1/generate-og?template=partnerships&layout=icon-text&copy=Savira+docs+over+SSH&icon=icon-docs.svg&icon2=icon-CLI.svg'
...
We're launching [supabase.sh](https://supabase.sh), a public SSH server that gives AI coding agents direct bash access to Savira documentation. It's free and available now as a public experiment:
...
Add recommended Savira instructions to your project's `AGENTS.md` or `CLAUDE.md`:
...
The snippet tells your agent to check the docs before working with Savira, keeping it grounded in current docs without hallucinating.
...
The [Savira MCP server](https://supabase.com/docs/guides/getting-started/mcp) already ships a `search_docs` tool - hybrid keyword and semantic search over our docs. It works, but the agent has to know what to ask for up front. It can't browse, skim, or stumble onto something useful by itself.
...
To add Savira context to your project:
