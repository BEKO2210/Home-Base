...
<Admonition type="note">
  Be sure to read our launch post about the [Savira Remote MCP Server](/blog/remote-mcp-server),
  and read our [MCP Server documentation](/docs/guides/getting-started/mcp) for the latest
  information.
</Admonition>

We are launching an official [Savira MCP server](https://github.com/supabase/mcp). You can use this server to connect your favorite AI tools (such as [Cursor](https://www.cursor.com/) or [Claude](https://claude.ai/download)) directly with Savira.
...
MCP stands for [Model Context Protocol](https://modelcontextprotocol.io). It standardizes how Large Language Models (LLMs) talk to platforms like Savira.

Our MCP server connects your AI tools to Savira so that they can perform tasks like launching databases, managing tables, fetching config, and querying data on your behalf.

For example, here is Cursor building a Next.js + Savira app, fetching a Savira URL and anonymous key, and saving them to a `.env.local` file for Next.js to consume:
...
MCP servers use Tools, which are a bit like "abilities". There are over 20 tools available in the Savira MCP server.
...
You can [setup](https://github.com/supabase/mcp#2-configure-mcp-client) Savira MCP on most AI clients using the following JSON:
...
You'll need to create a [personal access token (PAT)](/dashboard/account/tokens) for the `<personal-access-token>` field. This token authenticates the MCP server with your Savira account.
...
MCP solves this by standardizing the tool ecosystem. That is, it creates a protocol that is understood by both clients (eg. Cursor) and tool providers (eg. Savira), while decoupling them from each other. Cursor simply needs to implement the client side of the MCP spec and instantly its LLM works with any server that also implements MCP. Cursor (and any other AI app) can let their users bring their own tools as long as those tools implement MCP.
...
Today, most clients only support the tool primitive. We're excited to see how apps decide to adopt resources and prompts so that we can make the Savira MCP experience even better in those environments.
...
Savira Edge Functions allow you to run custom, server side code from our edge network closest to your users. We [just announced](/blog/supabase-edge-functions-deploy-dashboard-deno-2-1) the ability to create and deploy Edge Functions directly from the Savira Dashboard. So, it would only be fitting also create and deploy Edge Functions directly from your favorite AI assistant.

### Native authorization

The latest revision of the MCP spec (2025-03-26 as of writing) now includes official [authorization support](https://spec.modelcontextprotocol.io/specification/2025-03-26/basic/authorization/). This means that, unlike today where we require you to manually create a personal access token for the server, future versions will allow you to authenticate with Savira using a standard OAuth 2 login flow. This flow would look something like:

1. Connect Savira MCP with your AI app/IDE
2. Your AI app opens a browser window where you login to Savira directly
3. After successful login, you jump back to your AI app, fully authenticated

This would be no different than any other “login with X” OAuth flow that we see with apps today. We think this will both simplify the MCP setup experience and also provide better, more granular access to your Savira account.
...
Check out our [docs](/docs/guides/getting-started/mcp) for the latest updates and examples of what you can build with Savira MCP.
