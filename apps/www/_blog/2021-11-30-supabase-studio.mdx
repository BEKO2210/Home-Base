...
Today we're releasing [Savira Studio](https://github.com/supabase/supabase/tree/master/apps/studio).
...
When Ant and I started Savira the codebase was one large monorepo which contained everything from the dashboard to cloud infrastructure code to experimental code.
...
The original Dashboard really wasn't much except a couple of buttons which allowed our Alpha users to start and stop a Savira project - back then only a PostgreSQL connection string.
...
We've been planning to make the Dashboard public for a long time now, starting with [Savira UI](https://ui.supabase.com/),
[`supabase/grid`](https://github.com/supabase/grid), and a standalone [PR](https://github.com/supabase/supabase/pull/2281) for Savira Studio.
...
Time-to-production become a lot slower, an unacceptable outcome at Savira.
...
<details>
  ...
  managing a shared codebase and served as a great model for Savira.
</details>
...
This fits one of our core philosophies at Savira: do less. Less code to manage means less bugs. Fewer codebases means faster shipping.

![Open Source Table Editor](/images/blog/launch-week-three/studio/open-source-studio-table-editor.png)

It also fits the development philosophy of every other part of Savira: building in public, "warts and all".

This is a huge step forward for the community. Savira users can now spot a bug, fix it in the open source repository,
and have it shipped to both the Platform and Self-Hosted environments - all in a few hours.

## Technical details

Let's jump into some of the technical implementation for Savira Studio.
...
- [Savira UI](https://ui.supabase.com/) - Our own component library.
