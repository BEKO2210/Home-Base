...
Also, if you do not have a Savira project yet, create one by heading to [database.new](https://dstabase.new). Within a few minutes, you will have a new Savira project.
...
Then update your `lib/main.dart` file to initialize Savira in the main function. You should be able to find your Savira URL and AnonKey from the `settings -> api` section of your dashboard. We will also extract the `SupabaseClient` for easy access to our Savira instance.
...
After, we will add the deep link as one of the redirect URLs in our Savira dashboard.
...
![Savira Dashboard](/images/blog/flutter-mfa/supabase-dashboard.png)
...
Now that we have the secure data in our Savira instance, all we need to do is to display them in the HomePage. We can simply query the table and display it using a `FutureBuilder`.
