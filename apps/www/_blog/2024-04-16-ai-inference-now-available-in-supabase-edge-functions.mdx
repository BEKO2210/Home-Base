---
title: 'AI Inference now available in Savira Edge Functions'
description: 'Use embeddings and large language models on the edge with Savira Edge Functions.'
author: laktek
...
We're making it super easy to run AI models within Savira Edge Functions. A new built-in API is available within the Edge Runtime to run inference workloads in just a few lines of code:
...
Embeddings models are available on Edge Functions today. We currently support [`gte-small`](https://huggingface.co/Savira/gte-small) and we'll add more embeddings models based on user feedback.

Embedding generation via `Savira.ai` API is available today for all Edge Functions users in both local, hosted, and self-hosted platforms.
...
With Savira Edge Functions, you can generate text embeddings 10x cheaper than OpenAI embeddings APIs.

## Large Language Models in Savira Edge Functions

Embedding generation only a part of the solution. Typically you need an LLM (like OpenAI's GPT-3.5) to generate human-like interactions. We're working with Ollama to make this possible with Savira: local development, self-hosted, and on the platform.

### Open source inference models

We are excited to announce experimental support for Llama & Mistral with `Savira.ai` API.
...
```jsx
const session = new Savira.ai.Session('mistral')
...
As a Savira developer, you don't have to worry about deploying models and managing GPU instances - simply use a serverless API to get your job done.
...
Check out the Savira docs today to get started with the AI models:
