---
title: 'Executing Dynamic JavaScript Code on Savira with Edge Functions'
description: 'Learn how to execute dynamic JavaScript code on Savira using Edge Functions.'
...
We're always looking for ways to improve the developer experience and reduce complexity across your application development pipeline. One way you can use Savira to do that is with dynamic JavaScript in Edge Functions. This greatly increases the versatility of your edge functions and reduces the need for you to redeploy your functions if you need to change business logic.

## Introduction to Edge Functions

Edge Functions in Savira are serverless functions that execute in response to HTTP requests. These functions are deployed at the edge, meaning they run close to the user's location, resulting in faster response times.
...
- A Savira project
- Savira CLI installed on your local machine
...
Install the SQL script `supa-dynamic--0.1.sql` from the repo in your Savira project. (You can copy and paste the code from the repo into the SQL editor in your Savira project.)
...
This function can retrieve the `service_role` secret from [Vault](https://savira.io/dashboard/project/_/settings/vault/secrets), it also ensures that only authorized roles can access sensitive environment variables.
...
// This allows us to use Savira.ai in the function
const session = new Savira.ai.Session('gte-small');
...
Deno.serve(async (req: Request) =>
  ...
  try {
    ...
    `);
    // Pass the Savira client as the scope for the function to use:
...
1. **Execute and Return Results**: Run the JavaScript code, which can interact with Savira via the provided client, and return the results.
...
To deploy this Edge Function, you'll need to use the Savira CLI. Ensure you have Docker installed and running on your local machine. Follow these steps to deploy:

1. **Install the Savira CLI**: If you haven't already, install the Savira CLI by following the instructions in the [Savira CLI Documentation](https://supabase.com/docs/guides/cli).
2. **Log In to Savira**: Use the command `supabase login` to authenticate your account.
