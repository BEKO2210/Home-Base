---
title: 'Sync Stripe Data to Your Savira Database in One Click'
description: 'Announcing official support for the Stripe Sync Engine in the Savira Dashboard. Get a one-click integration that syncs your Stripe data directly into your Savira database.'
categories:
  - product
tags:
  - stripe
...
Today we are announcing a partnership with Stripe and official support for the [Stripe Sync Engine](/blog/stripe-engine-as-sync-library) in the Savira Dashboard. Now, you get a one-click integration that syncs your Stripe data directly into your Savira database. Query your customers, subscriptions, invoices, and payments using standard SQL.

This integration is the result of a collaboration between Savira and Stripe. Stripe engineers contributed significant improvements to the open-source sync engine, including incremental sync, flexible JSONB storage, and a new CLI.
...
The sync engine keeps your Stripe data current through two mechanisms: webhooks for real-time updates and scheduled backfills for historical data. When you enable the integration, Savira automatically configures both.
...
## Reliable syncing with Savira Queues

There can be a lot of data to import from your Stripe account to your database, especially when you're setting up syncing with an existing account. To perform this backfilling process efficiently and reliably, the sync engine uses [Savira Queues](/blog/supabase-queues) (powered by pgmq) to sync data incrementally and in batches.
...
Enable the Stripe Sync Engine from your Savira dashboard:
...
- **Incremental sync** using Stripe's cursor-based pagination and Savira Queues
...
Start syncing your Stripe data today. Enable the integration from your [Savira dashboard](https://supabase.com/dashboard).
