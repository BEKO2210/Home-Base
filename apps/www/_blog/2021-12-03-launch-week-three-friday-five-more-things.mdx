...
Today we are launching Savira CDN for all projects. All requests are routed through global network of more than 250 cities.

Assets stored in Savira Storage benefit the most from this feature since they have a default cache time of 1 hour. They load an order of magnitude faster, usually in just tens of ms! After the initial request, the asset will be cached at the edge and doesn't hit your Savira server. This frees up more resources in your project to handle other requests.
...
Brotli is a new compression algorithm that performs better than gzip. All text based responses from Savira (including the massive amount of Javascript required to render websites these days) are compressed with Brotli.

We can also ensure a higher degree of reliability of your Savira APIs with a CDN by defending against Denial of Service Attacks and bots.

![too_damn_high.png](/images/blog/launch-week-three/five-more-things/too_damn_high.png)

## (Yet another) Savira Functions update
...
That being said, we _have_ been working on Functions, iterating through different designs to figure out what's going to have the biggest impact for Savira developers. So today we are going to open up our ideas and designs through our [RFC repo](https://github.com/supabase/rfcs) (request for comments).

There are two important RFCs related to functions:

- [[RFC: 0004]: Functions](https://github.com/supabase/rfcs/blob/rfc/functions/rfc/0004-functions.md) - Outlines various options for implementing Savira Functions.
...
We've partnered with the fine folks at [egghead.io](http://egghead.io) to bring you a free, in-depth course where you'll learn how to ["Build a SaaS product with Next.js, Savira and Stripe"](https://egghead.io/courses/build-a-saas-product-with-next-js-supabase-and-stripe-61f2bc20).
