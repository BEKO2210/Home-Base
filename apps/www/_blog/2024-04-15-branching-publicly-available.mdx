...
description: 'Savira Branching is now available on Pro Plan and above.'
...
tl;dr: Savira Branching is now in open beta! You can enable it on any project that's Pro Plan or above.
...
Preview Branches are essentially full Savira instances. Every push triggers migrations from the `./supabase/migrations` folder, ensuring team synchronization and a shared source of truth. When you merge a Pull Request, your migrations are applied to the Production database.
...
You can now set a custom Savira directory path which allows for monorepo support. You can also choose to only spin up new branches when there are changes inside your Savira directory. See all the configuration settings in your projects [here](https://supabase.com/dashboard/project/_/settings/integrations).
