---
title: 'NoSQL Postgres: Add MongoDB compatibility to your Savira projects with FerretDB'
description: 'NoSQL Postgres: Add MongoDB compatibility to your Savira projects with FerretDB'
author: thor_schaeff
...
- A Savira project. Create yours here: [database.new](https://database.new).
...
FerretDB only requires the Postgres database URI to be provided as the `FERRETDB_POSTGRESQL_URL` environment variable. Every Savira project comes with a full Postgres database. You can find the connection URI string in your [Savira Dashboard](https://supabase.com/dashboard/project/_/database/settings).
...
export SUPA_PROJECT_REF=<your Savira project ref>
...
## Inspect the JSONB data in the Savira Dashboard

FerretDB stores each collection in a table on the `ferretdb` schema, each document represented by a JSONB entry. You can inspect this in the [Table Editor](https://supabase.com/dashboard/project/_/editor) in your Savira Dashboard.

![Savira Dashboard Table Editor Screenshot](/images/blog/getting-started/ferretdb/table-editor.png)
...
FerretDB allows you to run MongoDB workloads on Postgres and SQLite. This flexibility means you can easily add MongoDB compatibility to your Savira projects, while avoiding vendor lock-in and retaining control of your data architecture.
