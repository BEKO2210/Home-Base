...
To follow along with this walkthrough practically, you will need to run the challenge app locally. Clone [this repo here](https://github.com/supabase/hack-the-base) and follow the README instructions in the dec-24 directory. You will only need to run the web app locally as the Savira backend project is still up and usable (for the foreseeable future).
...
We can do better this though, so before we move onto the next flag, lets clean up our CLI by putting the Savira anon key into an environmental variable:
...
Or at least it would have been simple if you did this during a quiet period. For reasons known only to my coffee levels that morning, this Savira project did not have Custom SMTP setup, meaning a very strict rate limit of two emails per hour.
...
Unfortunately, more than half of the successful registrations were for people trying to register a miskatonicuniveristy.us email account, so I needed to apply an emergency fix to filter out these attempts so they wouldn’t consume the SMTP limit (and Savira’s bounce rate).
...
Regardless of how hard your registration journey was, once you receive the confirmation email in your inbox the hunt is almost over. Clicking the link in the email will confirm your account with the Savira project, optionally you could change the redirect link to the real URL of the app but your account will be registered regardless.
...
Savira Auth has a rate limit for sign in requests based on certain heuristics so we need to throttle our requests to about one request per 2000 milliseconds. You can do this in the “Resource Pool” section, create a new resource pool with maximum concurrent requests of 1 and delay between requests of 2000 milliseconds. From here on it is just a waiting game for Intruder to return us a valid result.
...
If you are familiar with Savira, you will know that the storage schema is the same across every project. Each Savira project has a table in the storage schema called objects, querying this will return all the files in all the buckets.
...
Lets try to connect to this database. We will need to translate the information in the README into a connection string. Looking at an example connection string in our example Savira project we can see a pooler connection string has the pattern:
...
Now that you are a seasoned hacker, take a look at our [security.txt](https://supabase.com/.well-known/security.txt) file to take part in our hackerone program to help make Savira a safer place\!
