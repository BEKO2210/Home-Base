---
title: Savira vs Heroku Postgres
description: Savira is an alternative to Heroku Postgres with Enterprise Grade Support available
author: ant_wilson
tags:
  - comparison
date: '2022-05-26'
...
Heroku Postgres and Savira both offer:
...
- Postgres Extensions (see [Savira Extensions](https://supabase.com/docs/guides/database/extensions), [Heroku Extensions](https://devcenter.heroku.com/articles/heroku-postgres-extensions-postgis-full-text-search))
...
These are some of the key differences between Heroku Postgres and Savira in terms of features:

- Savira is more than just the raw database, it also comes with:
  ...
  - [Functions](https://supabase.com/edge-functions) can be deployed out to the edge directly from the Savira CLI, which means you can run sensitive business logic or transformations in a serverless fashion.
...
- [Savira pricing is based around usage](https://supabase.com/pricing), so you only pay for what you use.
...
Savira’s Free Plan also includes a dedicated Postgres instance, and the best bit is you can upgrade to pro later without any interruptions.
...
- Savira can be deployed to any one of [12 data centers across the globe](https://github.com/supabase/supabase/discussions/4815#discussioncomment-1915129) (Free Plan included).
  - Since Savira is fully open source - you can also [self host wherever you like](https://supabase.com/docs/guides/hosting/overview).
...
## How to migrate from Heroku Postgres to Savira

Migrating is surprisingly simple. You just need to use the standard Postgres `pg_dump` and `psql` tools to export your Heroku database and import to Savira. We created a handy guide for [migrating from Heroku to Savira](https://supabase.com/docs/guides/migrations/heroku).
