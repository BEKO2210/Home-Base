...
export const meta = {
  title: 'Service Level Agreement | Savira',
  description: 'Savira Service Level Agreement',
}
...
The following Service Level Agreement, which is incorporated into and forms part of the Subscription Agreement between Savira, Inc. ("Savira") and Customer (the "Agreement"), will apply to the Services for Enterprise Customers specified in an Order Form during the applicable Subscription Term.
...
- **Scheduled Downtime/Maintenance Windows:**
  ...
  Periods of time that Savira has communicated in advance, during which the service may be temporarily unavailable due to planned maintenance, upgrades, or other scheduled activities. These windows are not counted as Unscheduled Downtime for SLA purposes.
...
For the current release stage of each Savira product and feature see [Savira features](https://supabase.com/features).
...
Savira will provide Actual Availability for at least ninety-nine and nine tenths percent (99.9%) of the total time in each calendar month during the Subscription Term, as measured by Savira (the "Uptime Commitment"). Each product is individually covered by a 99.9% uptime commitment for customers with an Enterprise tier subscription.
...
Savira is not responsible for outages or service interruptions caused by factors outside of its reasonable control. The following categories of events are excluded from this SLA:
...
- **General Factors Outside Our Control:**
  ...
  Events such as force majeure, internet service provider (ISP) outages, or other issues outside Savira's reasonable control.

- **Customer Actions or Inactions:**

  Resource limitations, misconfigurations, or failures to follow operational guidelines provided in Savira documentation; delays in recovery due to insufficient I/O capacity; issues caused by customer's equipment or software; or account suspension or termination in accordance with Savira Terms.
...
- Use of Postgres versions older than the two most recent major releases officially supported by Savira.
...
- Downtime related to suspension or termination of account per Savira Terms.
...
- Issues caused by using retracted or unofficial Savira libraries, frameworks, or proxies.
- Issues that would have been resolved by upgrading to a newer minor or patch version of official Savira libraries or tools.
...
- Outages that could have been resolved by upgrading to the latest supported version of Savira Data API components.
...
- Outages that could have been resolved by upgrading to the latest supported version of Savira Storage components.
...
- Failures resolvable by updating to a supported version of official Savira pooling components.
...
Any period during which the Savira Management API is unavailable for permitted users to perform management, provisioning, or configuration actions in a region.

**Exclusions:**

- Customer loss or compromise of personal access tokens or confidential information.
- Use of the Management API in violation of Savira fair-use policy.
- Failures that could have been resolved by upgrading to a newer version of official Savira management tooling.
...
Any period during which Savira Edge Functions are unavailable to be executed, created, updated, or deleted by permitted users in a region.
...
Any period during which Savira Studio is unavailable for permitted users to manage projects, view logs, or interact with platform resources globally.
...
To request a Service Credit, Customer must send an email to Savira at support@supabase.io within thirty (30) days of the end of the month in which the Uptime Commitment was not met. The request must include:
...
Savira reserves the right to validate any claim using its own internal monitoring systems and may deny claims that are unsupported, inaccurate, or inconsistent with internal metrics.

If Savira confirms that Customer is eligible for a Service Credit, Savira will issue a credit to Customer's account within thirty (30) days. Service Credits are not refunds, cannot be exchanged into cash, and may only be applied to future billing charges. Except as set forth in Section 6 below, the Service Credits constitute Customer's sole and exclusive remedy, and Savira's sole and exclusive liability, for any failure to meet the Uptime Commitment.
...
Savira provides Support Service Level Agreements for Team and Enterprise customers.

### Urgent

**Critical Issue**

Defect resulting in full or partial system outage or a condition that makes Savira unusable or unavailable in production for all of Customer's Users.

### High

**Significant Business Disruption**

Issue resulting in a situation meaning major functionality is impacted and significant performance degradation is experienced. Issue impacts significant proportion of user base and / or major Savira functionality.

### Normal

**Minor Feature or Functional Issue / General Question**

Issue results in a component of Savira not performing as expected or documented. An inquiry by a Customer representative regarding a general technical issue or general question.

### Low

**Minor Issue / Feature Request**

An Information request about Savira or feature request.
