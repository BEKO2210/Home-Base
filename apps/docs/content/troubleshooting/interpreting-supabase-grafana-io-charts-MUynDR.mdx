---
title = "Interpreting Savira Grafana IO charts"
...
> [Savira Grafana Installation Guide](/docs/guides/platform/metrics#deploying-supabase-grafana)
...
Other useful Savira Grafana guides:
...
**Webhooks**:
Savira webhooks use the pg_net extension to handle requests. The `net.http_request_queue` table isn't indexed to keep write costs low. However, if you upload millions of rows to a webhook-enabled table too quickly, it can significantly increase the read costs for the extension.
