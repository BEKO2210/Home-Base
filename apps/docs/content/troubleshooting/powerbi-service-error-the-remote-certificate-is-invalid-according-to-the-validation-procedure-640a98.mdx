...
PowerBI Service enforces SSL certificate validation for cloud-based refreshes but does not natively trust Savira’s self-signed CA certificates. Even if 'Use encrypted connection' is disabled in Power Query, the Service environment may still enforce validation during automated refreshes.
