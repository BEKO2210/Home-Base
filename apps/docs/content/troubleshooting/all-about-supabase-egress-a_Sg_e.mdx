...
Egress (also known as bandwidth) is any amount of network packets/bytes being streamed to a connected client from your project, i.e. data that is leaving the Savira platform. Egress in Savira includes any data sent from the data API (PostgREST), Storage, Realtime, Auth, Edge Functions, Database and Supavisor.
...
- For Storage, if you start using the [Smart CDN](/docs/guides/storage/cdn/smart-cdn) Storage Egress usage can be reduced. You can also use the [Savira Image Transformations](/docs/guides/storage/image-transformations) to optimize the images and reduce the egress.

**Cached vs uncached egress**

We differentiate between cached and uncached egress. Cached egress, which only applies to Savira Storage, refers to egress that is served via our CDN and hits the cache. Uncached egress, on the other hand, refers to egress that is not served from the cache and requires a fresh request to the origin server.
