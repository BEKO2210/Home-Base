...
The same constraints placed on edge function's hosted by Savira are also imposed by the test environment spun-up by the CLI. You can follow the function's [local development guide](/docs/guides/functions/quickstart) to set up a test environment and then serve your function locally:
...
If you are performing logic to process data from Savira Postgres, you may be able to handle the processing within the database directly by using [database functions](/docs/guides/database/functions?queryGroups=language&language=js) or refactored queries.
...
If you have functions that call other functions, always implement an escape condition. Savira will terminate functions that recursively self-call past a certain depth, but your code should enforce its own limit.
