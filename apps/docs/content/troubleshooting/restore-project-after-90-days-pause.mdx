...
Projects paused for more than 90 days can no longer be restored through Savira Studio. You can still recover your data by downloading the available backups and migrating them to a new project.

Both the database backup and Storage objects can be downloaded from the [Project Overview](/dashboard/project/_) section in Savira Studio before the project is deleted.
...
## Step 2: Create a new Savira project
...
Some errors like `object already exists` are expected and can be safely ignored — they occur because the new project already has the default Savira schemas applied.
...
Use the [Savira CLI](/docs/reference/cli/introduction) to copy your downloaded storage files to the new project's buckets:
...
# ---------------------------------------------------------------------------
# Sync Savira project configuration from a SOURCE project to a TARGET project
# using the Savira Management API.
#
# Usage:
...
- [Migrating within Savira](/guides/platform/migrating-within-supabase)
