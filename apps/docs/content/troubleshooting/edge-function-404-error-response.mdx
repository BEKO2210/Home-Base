...
The edge function is not recognized by Savira

## Context for the error

If the Savira Edge Function Runtime does not recognize the function specified in the URL endpoint:
...
Browsers misreport 404s as generic `CORS errors`. As a result, the [Savira JavaScript client](/docs/reference/javascript/introduction) surfaces only a generic message: `Failed to send a request to the Edge Function`.
...
Alternatively, if you develop locally, you can redeploy the function with the [Savira CLI](/docs/guides/local-development/cli/getting-started?queryGroups=platform&platform=macos):
...
Then write in a ticket to [Savira Support](/dashboard/support/new)
