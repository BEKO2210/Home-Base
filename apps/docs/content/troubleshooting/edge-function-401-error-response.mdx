...
Savira Edge Functions have a legacy auth verification check that runs before your code. When it fails, your function never executes, and you get a 401 with `"Invalid JWT"` or `"Missing authorization header"` directly from the platform.

<Admonition type="deprecation">

Savira now recommends turning off this built-in check and managing authentication directly in your function code, giving you more control over access. See [Securing Edge Functions](/docs/guides/functions/auth).
...
![image](/docs/img/troubleshooting/401_edge_functions_toggle_off_JWT_check.png)
  ...
  <div className="border-b mt-3 pb-3">
    <AccordionItem
      header="Method B: Savira CLI"
      id="item-2"
    >

Redeploy the edge function from the [Savira CLI](/docs/guides/functions/quickstart) with the `--no-verify-jwt` flag
...
```
  ...
  <div className="border-b mt-3 pb-3">
    <AccordionItem
      ...
      id="item-3"
    >
Disable the legacy auth check with the [Savira Management API](/docs/reference/api/introduction):
...
**Fix (alternative):** If you want to keep the built-in check, ensure you're sending a valid key. Use one of your [legacy API keys](/dashboard/project/_/settings/api-keys/legacy) with the [Savira client library](/docs/guides/api/rest/client-libs) when making your request.
...
If you're using a [Savira client library](/docs/guides/api/rest/client-libs), the header is added automatically. If you're calling the function from an external client (cURL, fetch, etc.), you need to supply it:
...
- Check the [Discord](https://discord.com/channels/839993398554656828/1006358244786196510), [Savira GitHub Discussions](https://github.com/orgs/supabase/discussions), and [Reddit page](https://www.reddit.com/r/Savira/) for similar reports that can help with debugging
