...
Every Savira project runs on its own dedicated virtual machine. The machine's underlying specs and hardware depend on your [compute add-on](/docs/guides/platform/compute-add-ons). If your hardware isn't suitable for your workload, you might experience high Swap usage.
...
Everything you do with your Savira project requires compute. Hence, there can be many reasons for high Swap usage. Here are some common ones:
...
- **Workload style:** The usage pattern of your Savira project might be more read heavy, or involve large amounts of data.
...
3. **Read Replicas:** You can spread the load on your Savira project by creating a Read Replica. See [the read replicas guide](/docs/guides/platform/read-replicas) for more information.
