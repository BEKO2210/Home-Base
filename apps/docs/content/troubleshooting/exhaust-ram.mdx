...
You may observe elevated memory usage even when your database has little to no load. Savira requires a wide range of services other than Postgres to operate, which can result in an elevated base memory usage. Especially on the smallest compute instance that comes with 1 GB of RAM, it is not unusual for your project to have a base memory usage of ~50%.

## Issues with high memory usage

Every Savira project runs in its own dedicated virtual machine. Your instance will have a different set of hardware provisioned depending on your [compute add-on](/docs/guides/platform/compute-add-ons). Depending on your workload, your compute hardware may not be suitable and can result in high RAM usage.
...
To check your RAM usage on the Savira Platform, head over to [Database Health in the Observability section](/dashboard/project/_/observability/database).
...
Everything you do with your Savira project requires memory in some form. Hence, there can be many reasons for high RAM usage. Here are some common ones:
