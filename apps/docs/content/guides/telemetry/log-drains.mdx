...
id: 'log-drains'
title: 'Log Drains'
description: 'Getting started with Savira Log Drains'
---

Log drains send all logs of the Savira stack to one or more desired destinations. It is only available for customers on Pro, Team and Enterprise Plans. Log drains are available in the dashboard under [Project Settings > Log Drains](/dashboard/project/_/settings/log-drains).
...
Logs sent to Datadog have the name of the log source set on the `service` field of the event and the source set to `Savira`. Logs are gzipped before they are sent to Datadog.
...
>
    ...
    2. Create log drain in [Savira dashboard](/dashboard/project/_/settings/log-drains)
...
Logs are sent to Sentry as part of [Sentry's Logging Product](https://docs.sentry.io/product/explore/logs/). Ingesting Savira logs as Sentry errors is currently not supported.
...
3. Create log drain in [Savira dashboard](/dashboard/project/_/settings/log-drains), providing:
   - Name of the dataset
   - API token
...
To receive Savira logs with the OpenTelemetry Collector, configure an OTLP HTTP receiver:
...
Then create a log drain in [Savira dashboard](/dashboard/project/_/settings/log-drains) with:
