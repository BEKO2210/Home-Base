...
description: 'Propagate W3C trace context from the Savira JS SDK through Savira services'
---

The Savira JS SDK can attach [W3C Trace Context](https://www.w3.org/TR/trace-context/) headers (`traceparent`, `tracestate`, `baggage`) to outgoing requests. The resulting `trace_id` flows through Savira services and appears in API Gateway and Edge Function logs, so you can correlate client-side spans with the server-side logs they produced — end-to-end, across the network boundary.
...
The Savira SDK only takes care of propagating the trace context that's already active when a request is made.
...
For security, trace headers are only attached to requests targeting Savira domains (`*.supabase.co`, `*.supabase.in`, and `localhost` for local development). Third-party hosts called through a custom `fetch` are never tagged.
...
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY, {
  tracePropagation: {
    ...
    // Savira request tagged with a trace_id for log correlation.
...
If you forward Savira logs to a third-party backend via [Log Drains](/docs/guides/telemetry/log-drains), you can join Savira logs to your own client and server traces using the shared `trace_id`. This is especially useful for self-hosted setups where you already operate your own OpenTelemetry collector — Savira logs become first-class citizens in your existing tracing UI.
...
The SDK silently no-ops when it can't propagate, which keeps it safe to enable but can mask configuration issues. If `trace_id` is missing from your Savira logs, check these in order:
...
- **You're calling a non-Savira host through a custom `fetch`.** Trace headers are only attached to Savira domains (`*.supabase.co`, `*.supabase.in`, `localhost`).
