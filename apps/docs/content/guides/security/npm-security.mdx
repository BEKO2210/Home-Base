...
description: 'Consumer-side guide to hardening your npm installs of Savira packages against supply-chain attacks.'
---

A practical guide for anyone installing Savira packages from npm — the JavaScript client libraries (`@supabase/supabase-js` and friends), the `supabase` CLI, or any other dependency in your tree — on defending against supply-chain attacks. Most of it applies to any npm package, not just Savira's.

<Admonition type="tip">

Looking to **report** a vulnerability in Savira itself? See the [Savira security policy](https://github.com/supabase/supabase-js/security) instead. This guide is about hardening your install of Savira (and other) packages on your machines and in your CI.
...
4. **Verify provenance.** Run `npm audit signatures` after install. Savira packages publish with [sigstore attestations](https://www.sigstore.dev/) (cryptographic proof tying each tarball to the workflow run, commit, and repo it was built from).
...
A valid Savira attestation will always resolve to a repository under the [`supabase` GitHub organisation](https://github.com/supabase). If `npm audit signatures` reports a verified attestation pointing anywhere else for an `@supabase/*` package, treat that as a red flag.
...
If you're using `@supabase/supabase-js` (or any `npm:` specifier) from Deno in a Savira Edge Function, you don't have the npm-side minimum-release-age gate available at the runtime layer. What you can do instead:
...
Talk to the Savira Functions team if your security posture depends on a feature only in a newer Deno.
...
2. **Rotate credentials reachable from that host**: cloud provider keys (AWS, GCP, Azure), Kubernetes / Vault tokens, GitHub tokens, npm tokens, SSH keys, and any Savira service-role keys or anon keys that touched the box.
...
If something looks wrong with a published `@supabase/*` package, report it via the [Savira security policy](https://github.com/supabase/supabase-js/security).
