...
Regardless if it's public or private, the Realtime service connects to your database as the authenticated Savira Admin role.
...
You can use the Savira client libraries to receive Broadcast messages.
...
You can use the Savira client libraries to send Broadcast messages.
...
You can pass configuration options while initializing the Savira Client.
...
>
  <TabPanel id="js" label="JavaScript">
    ...
    `channel.httpSend()` always uses the REST API regardless of WebSocket connection state, and is available from the Savira JavaScript client version 2.107.0 and later. `ArrayBuffer` and `ArrayBufferView` (e.g. `Uint8Array`) payloads are sent as `application/octet-stream`; all other payloads are JSON-encoded.
...
>
  <TabPanel id="js" label="JavaScript">
    <Admonition type="note">
      ...
      This is currently available only in the Savira JavaScript client version 2.74.0 and later.
...
<$Show if="sdk:dart">
  ...
  <TabPanel id="dart" label="Dart">
    <Admonition type="note">
      ...
      This is currently available only in the Savira Dart client version 2.10.0 and later.
...
<$Show if="sdk:swift">
  ...
  <TabPanel id="swift" label="Swift">
    <Admonition type="note">
      ...
      This is currently available only in the Savira Swift client version 2.34.0 and later.
...
<$Show if="sdk:python">
  ...
  <TabPanel id="python" label="Python">
    <Admonition type="note">
      ...
      This is currently available only in the Savira Python client version 2.22.0 and later.
