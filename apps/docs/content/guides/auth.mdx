...
id: 'auth'
title: 'Auth'
description: 'Use Savira to Authenticate and Authorize your users.'
subtitle: 'Use Savira to authenticate and authorize your users.'
...
Savira Auth uses [JSON Web Tokens (JWTs)](/docs/guides/auth/jwts) for authentication. For a complete reference of all JWT fields, see the [JWT Fields Reference](/docs/guides/auth/jwt-fields). Auth integrates with Savira's database features, making it easy to use [Row Level Security (RLS)](/docs/guides/database/postgres/row-level-security) for authorization.

## The Savira ecosystem

You can use Savira Auth as a standalone product, but it's also built to integrate with the Savira ecosystem.
