...
subtitle: 'Learn how to configure automatic retries for your Savira API requests.'
---
...
// Create a Savira client instance with the custom fetch
...
### 4. Using the Savira client

With `fetch-retry` integrated, you can use the Savira client as usual. The retry logic will automatically apply to all network requests made by `supabase-js`.
