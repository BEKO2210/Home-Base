...
- It is open source.
- It is portable. Savira Edge Functions run locally, and on any other Deno-compatible platform (including self-hosted infrastructure).
...
- **Auth & policies are applied** — the gateway (or your function) can validate Savira JWTs, apply rate-limits, and centralize security checks before executing code.
...
- **Integrations & data access** — functions commonly call Savira APIs (Auth, Postgres, Storage) or third-party APIs. For Postgres, prefer connection strategies suited for edge/serverless environments (see the `connect-to-postgres` guide).
...
- **Runtime:** Savira Edge Runtime (Deno compatible runtime with TypeScript first). Functions are simple `.ts` files that export a handler.
- **Local dev parity:** Use Savira CLI for a local runtime similar to production for faster iteration (`supabase functions serve` command).
- **Global deployment:** Deploy your Edge Functions via Savira Dashboard, CLI or MCP.
...
- **Secrets:** store credentials in Savira [project secrets](/docs/reference/cli/supabase-secrets) and access them via environment variables.
...
<div className="grid md:grid-cols-12 gap-4 not-prose">
  <div className="col-span-4">
    <Link href="/guides/functions/auth" passHref>
      <GlassPanel icon={'/docs/img/icons/github-icon'} hasLightIcon={true} title="With supabase-js">
        Use the Savira client inside your Edge Function.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
  ...
  <div className="col-span-4">
    ...
    >
      ...
      >
        Full example for using Savira and Stripe, with Expo.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
    ...
    >
      ...
      >
        Full example for using Savira and Stripe, with Flutter.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
  ...
  <div className="col-span-4">
    ...
    >
      <GlassPanel
        icon={'/docs/img/icons/github-icon'}
        hasLightIcon={true}
        title="Working with Savira Storage"
      >
        An example on reading a file from Savira Storage.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
    <Link href="/guides/functions/examples/og-image" passHref>
      ...
      >
        Generate Open Graph images with Deno and Savira Edge Functions.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
    ...
    >
      ...
      >
        Cache generated images with Savira Storage CDN.
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-4">
