...
title: 'Build a Savira Integration'
subtitle: "This guide steps through building a Savira Integration using OAuth2 and the management API, allowing you to manage users' organizations and projects on their behalf."
description: 'Build a Savira Integration using OAuth2 and the Management API.'
---
...
## Show a "Connect Savira" button

In your user interface, add a "Connect Savira" button to kick off the OAuth flow. Follow the design guidelines outlined in our [brand assets](/brand-assets).

## Implementing the OAuth 2.0 flow

Once you've published your OAuth App on Savira, you can use the OAuth 2.0 protocol get authorization from Savira users to manage their organizations and projects.

You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using Savira Edge Functions [on our GitHub](https://github.com/supabase/supabase/tree/master/examples/edge-functions/supabase/functions/connect-supabase).
...
- `redirect_uri`: The URL where Savira will redirect the user to after providing consent.
...
Once the user consents to providing API access to your OAuth App, Savira will redirect the user to the `redirect_uri` provided in the previous step. The URL will contain these query parameters:

- `code`: An authorization code you should exchange with Savira to get the access and refresh token.
...
- `client_secret`: The secret that authenticates your OAuth App to Savira.
...
If the user has revoked access to your application, you will not be able to refresh a token. Furthermore, access tokens will stop working. Make sure you handle HTTP Unauthorized errors when calling any Savira API.
