...
title: 'Savira Partner Integration Guide'
subtitle: 'Integrate Savira with your platform or product.'
description: 'This guide explains how a Savira partner can integrate with the Savira platform via OAuth.'
---

This guide assumes you have already followed [the Build a Savira Integration guide](/docs/guides/integrations/build-a-supabase-oauth-integration) and have a working OAuth client.

When a Savira user clicks the **Install Integration** button in the Dashboard, they are redirected to your system to begin the OAuth flow. You can implement this redirect in one of two ways:

- **[Simple redirect](#method-1-simple-redirect)**: Easier to build, but your system cannot verify that the incoming user was sent by Savira.
- **[Signed redirect](#method-2-signed-redirect)**: More work to build, but cryptographically verifies that the redirect originated from Savira. **Recommended for production integrations.**

Pick the method that fits your security requirements, then follow the matching section below.

## Method 1: Simple redirect

In this method, you implement a single `GET` endpoint. Savira redirects the user to this endpoint when they click **Install Integration**, and your endpoint kicks off the OAuth flow.
...
sequenceDiagram
  ...
  participant Savira as Savira (OAuth Authorization Server/Protected Resource)

  Note over Browser: Savira Dashboard Open

  User->>Browser: Clicks "Install Integration"

  Browser->>Partner: "Connect to Partner" Click Handler Page
  Partner->>Browser: Redirect to Savira Authorization Page

  Browser->>Savira: Request Authorization
  Savira->>Browser: Redirect to Consent Screen

  Browser->>User: Display Consent Screen
  User->>Browser: Gives Consent

  Browser->>Savira: Submit User Consent
  Savira->>Browser: Redirect to Partner With Authorization Code

  Browser->>Partner: Submit Authorization Code

  Partner->>Savira: Exchange Authorization Code for Token
  Savira->>Partner: Return Token

  Partner->>Savira: Request Management API Resources
  Savira->>Partner: Return Resources
...
Savira will append the following query parameters to the URL when redirecting:

| Param               | Description                                                                                    |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| `project_id`        | Savira project ref of the project where the user clicked the **Install Integration** button. |
| `organization_slug` | Savira organization slug where the user clicked the **Install Integration** button.          |
...
Your endpoint may ask the user to sign up, sign in, or perform other setup tasks on your website. Once those are complete, immediately [redirect to the Savira authorization URL](/docs/guides/integrations/build-a-supabase-oauth-integration#redirecting-to-the-authorize-url) without further user interaction. This starts the OAuth flow.

### Step 2: Share your endpoint with Savira

Send Savira the URL of your endpoint so we can configure the **Install Integration** button to redirect there.

## Method 2: Signed redirect

In this method, Savira signs a JWT before redirecting the user. You verify the signature, generate a one-time redirect record, and return its URL to Savira. Savira then redirects the user to that URL. This guarantees that any redirect your system handles originated from Savira.
...
sequenceDiagram
  ...
  Browser->>Savira: Request Redirect URL Generation
  Savira->>Partner: Send Signed JWT
  Partner->>Partner: Validate JWT
  Partner->>Partner: Generate Unique Redirect URL
  Partner->>Savira: Return Redirect URL
  Savira->>Browser: Redirect to the Redirect URL
...
### Step 1: Exchange public keys with Savira

Savira generates two key-pairs, one for staging, one for production, and shares the public keys with you. Save both public keys and their key IDs in your system. The keys are PEM-encoded EC P-256.
...
Storing keys by ID enables zero-downtime key rotation. When Savira rotates keys, we share new key-pairs and start signing JWTs with the new key ID. Your system picks the correct key based on `kid` without any code changes.
...
This endpoint receives the signed JWT from Savira and returns a one-time redirect URL.
...
| `organization_slug` | No       | The Savira organization the user is connecting from. Use to pre-select the org during the OAuth consent screen. |
| `project_id`        | No       | The Savira project ref the user wants to connect. Use to pre-select a Savira project in your UI.              |
...
5. Verify that `aud` matches the value you agreed with Savira.
...
4. Redirect the user to the [Savira authorization URL](/docs/guides/integrations/build-a-supabase-oauth-integration#redirecting-to-the-authorize-url) to start the OAuth flow.

<Admonition type="tip">

If you need the user to perform setup steps on your site, design the experience as a wizard that ends by redirecting to the Savira authorization URL. This minimizes the chance of users getting distracted, navigating elsewhere on your site, and abandoning the OAuth flow.

</Admonition>

### Step 4: Share your details with Savira

Send Savira the following so we can configure the **Install Integration** button:
...
- The `aud` claim value you want Savira to send in the signed JWT.

Ask Savira for the public keys and key IDs for the staging and production environments.
