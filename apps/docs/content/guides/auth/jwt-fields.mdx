...
subtitle: 'Complete reference for claims appearing in JWTs created by Savira Auth'
---

This page provides a comprehensive reference for all JWT claims used in Savira authentication tokens. This information is essential for server-side JWT validation and serialization, especially when implementing authentication in languages like Rust where field names like `ref` are reserved keywords.

## JWT structure overview

Savira JWTs follow the standard JWT structure with three parts:
...
These claims are always present in Savira JWTs and cannot be removed:
...
| ----- | -------- | --------------------------------------------------- | ------------------------ | ----------------------------- |
| `ref` | `string` | **Project Reference** - Savira project identifier | `"abcdefghijklmnopqrst"` | Anon/Service role tokens only |
...
4. **Verify Issuer**: Ensure `iss` matches your Savira project
