...
description: 'Learn how to implement social authentication in an app with Expo React Native and Savira Database and Auth functionality.'
---
...
- [Savira Database](/docs/guides/database) - a Postgres database for storing your user data with [Row Level Security](/docs/guides/auth#row-level-security) to ensure data is protected and users can only access their own information.
- [Savira Auth](/docs/guides/auth) - enables users to log in through social authentication providers (Apple and Google).

![Savira Social Auth example](/docs/img/supabase-expo-social-auth-login.png)
...
These variables are safe to expose in your Expo app since Savira has [Row Level Security](/docs/guides/database/postgres/row-level-security) enabled on your database.
...
By default Savira Auth requires email verification before a session is created for the user. To support email verification you need to [implement deep link handling](/docs/guides/auth/native-mobile-deep-linking?platform=react-native)!
...
Now integrate social authentication with Savira Auth, starting with Apple authentication.
...
And add it to the **Redirect URLs** field in [your Savira dashboard Authentication configuration](/dashboard/project/_/auth/url-configuration).

For more information, follow the [Savira Login with Apple](/docs/guides/auth/social-login/auth-apple) guide.
...
- Relying on the [`signInWithOAuth`](/docs/reference/javascript/auth-signinwithoauth) function of the Savira Auth - that also supports iOS, Android and Web (useful also to manage any other OAuth provider)
...
>
  ...
  <TabPanel id="mobile" label="Mobile">
    ...
    ![Savira Social Auth example](/docs/img/supabase-expo-social-auth-tabs.png)
