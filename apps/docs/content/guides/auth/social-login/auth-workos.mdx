...
## Step 3. Add your WorkOS credentials to your Savira project

![Enter your WorkOS application details in your Savira app's auth provider settings panel](/docs/img/guides/auth-workos/supabase-workos-configuration.png)
...
## Step 4. Set your Savira redirect URI in the WorkOS Dashboard

Visit the WorkOS dashboard and click the redirects button in the left navigation panel.

On the redirects page, enter your Savira project's `Callback URL (for OAuth)` which you saved in the previous step, as shown below:
...
async function signInWithWorkOS() {
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'workos',
    options: {
      redirectTo: 'http://example.com/auth/v1/callback', // Make sure your redirect URL is configured in the Savira Dashboard Auth settings
      queryParams: {
