...
description: 'Add Slack OAuth to your Savira project'
---

To enable Slack Auth for your project, you need to set up a Slack OAuth application and add the application credentials to your Savira Dashboard.
...
- Add your Slack `API Key` and `API Secret Key` to your [Savira Project](/dashboard).
...
- Add the following scopes under the `User Token Scopes`: `profile`, `email`, `openid`. These scopes are the default scopes that Savira Auth uses to request for user information. Do not add other scopes as [Sign In With Slack only supports `profile`, `email`, `openid`](https://api.slack.com/authentication/sign-in-with-slack#request).

## Enter your Slack credentials into your Savira project
