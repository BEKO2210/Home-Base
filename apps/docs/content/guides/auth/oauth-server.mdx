...
description: 'Turn your Savira project into an OAuth 2.1 and OpenID Connect identity provider'
---

Savira Auth can act as an OAuth 2.1 and OpenID Connect (OIDC) identity provider. This allows other applications and services to use your Savira project as their authentication provider, just like "Sign in with Google" or "Sign in with GitHub".
...
There are several reasons why you might want to enable OAuth 2.1 Server in your Savira project:
...
Savira Auth implements the OAuth 2.1 authorization code flow with PKCE (Proof Key for Code Exchange). When a third-party application wants to access user data:

1. The application redirects the user to your authorization endpoint
2. Savira Auth validates the request and redirects to your custom authorization UI
3. The user authenticates (using any of your enabled auth methods) and approves access
4. Savira Auth issues an authorization code
...
Access tokens are standard Savira JWTs that include `user_id`, `role`, and `client_id` claims. Your existing Row Level Security policies automatically apply to OAuth tokens, giving you fine-grained control over what each client can access.
...
OAuth 2.1 Server works seamlessly with your existing Savira Auth configuration:
...
- All standard Savira features (email templates, hooks, rate limiting) continue to work
