...
subtitle: 'Information on how best to use JSON Web Tokens with Savira'
---
...
Savira Auth continuously issues a new JWT for each user session, for as long as the user remains signed in. Check the comprehensive guide on [Sessions](/docs/guides/auth/sessions) to find out how you can tailor this process for your needs.

JWTs provide the foundation for [Row Level Security](/docs/guides/database/postgres/row-level-security). Each Savira product is able to securely decode and verify the validity of a JWT it receives before using Postgres policies and roles to authorize access to the project's data.

Savira provides a comprehensive system of managing [JWT Signing Keys](/docs/guides/auth/signing-keys) used to create and verify JSON Web Tokens.
...
Savira creates JWTs in these cases for you:

1. **When using Savira Auth, an access token (JWT) is created for each user while they remain signed in**. These are short lived, so they are continuously issued as your user interacts with Savira APIs.
...
In addition to creating JWTs, Savira can also accept JWTs from other authentication servers via the [Third-Party Auth](/docs/guides/auth/third-party/overview) feature or ones that have been minted externally via an imported [JWT Signing Key](/docs/guides/auth/signing-keys).

## Using custom or third-party JWTs

<Admonition type="note">

The `supabase.auth.getClaims()` method is meant to be used only with JWTs issued by Savira Auth. If you mint your own JWTs using a key you've imported, the verification may fail. We strongly recommend using a JWT verification library for your language to verify this type of JWT based on the claims you're adding in them.

</Admonition>

Your Savira project accepts a JWT in the `Authorization: Bearer <jwt>` header. If you're using the Savira client library, it does this for you.

If you are already using Savira Auth, when a user is signed in, their access token JWT is automatically managed and sent for you with every API call.
...
In the past there was a recommendation to set custom headers on the Savira client with the `Authorization` header including your custom JWT. This is no longer recommended as it's less flexible and causes confusion when combined with a user session from Savira Auth.

## Verifying a JWT from Savira

If you're not able to use the Savira client libraries, the following can be used to help you securely verify JWTs issued by Savira.

Savira Auth exposes a [JSON Web Key](https://datatracker.ietf.org/doc/html/rfc7517) Set URL for each Savira project:
...
Below is an example of how to use the [jose TypeScript JWT verification library](https://github.com/panva/jose) with Savira JWTs:
...
- [JWT Claims Reference](/docs/guides/auth/jwt-fields) - Complete reference for all JWT claims used by Savira Auth
