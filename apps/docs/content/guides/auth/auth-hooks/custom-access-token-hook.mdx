...
subtitle: 'Customize the access token issued by Savira Auth'
---
...
Claims returned must conform to our specification. Savira Auth will check for these claims after the hook is run and return an error if they are not present.
