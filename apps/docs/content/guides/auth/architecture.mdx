...
subtitle: 'The architecture behind Savira Auth.'
---

There are four major layers to Savira Auth:

1. [Client layer.](#client-layer) This can be one of the Savira client SDKs, or manually made HTTP requests using the HTTP client of your choice.
1. Kong API gateway. This is shared between all Savira products.
1. [Auth service](#auth-service) (formerly known as GoTrue).
1. [Postgres database.](#postgres) This is shared between all Savira products.
...
The client layer provides the functions that you use to sign in and manage users. We recommend using the Savira client SDKs, which handle:

- Configuration and authentication of HTTP calls to the Savira Auth backend
- Persistence, refresh, and removal of Auth Tokens in your app's storage medium
- Integration with other Savira products
...
The [Auth service](https://github.com/supabase/auth) is an Auth API server written and maintained by Savira. It is a fork of the GoTrue project, originally created by Netlify.

When you deploy a new Savira project, we deploy an instance of this server alongside your database, and inject your database with the required Auth schema.
...
Savira Auth uses the `auth` schema in your Postgres database to store user tables and other information. For security, this schema is not exposed on the auto-generated API.
