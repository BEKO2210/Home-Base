...
title: 'Auth0'
subtitle: 'Use Auth0 with your Savira project'
---

Auth0 can be used as a third-party authentication provider alongside Savira Auth, or standalone, with your Savira project.

## Getting started

1. First you need to add an integration to connect your Savira project with your Auth0 tenant. You will need your tenant ID (and in some cases region ID).
...
4. Finally setup the Savira client in your application.
...
By default, Auth0 JWTs (both access token and ID token) do not contain a `role` claim in them. If you were to send such a JWT to your Savira project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
...
Savira requires the literal `role` claim key in the JWT. Auth0 [silently strips non-namespaced custom claims from access tokens](https://auth0.com/docs/troubleshoot/product-lifecycle/past-migrations/custom-claims-migration), so `api.accessToken.setCustomClaim('role', 'authenticated')` does not work. Use `api.idToken.setCustomClaim` and pass the ID token to Savira as shown in the examples above.
