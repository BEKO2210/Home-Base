...
subtitle: 'Use Amazon Cognito via Amplify or standalone with your Savira project'
---

Amazon Cognito User Pools (via AWS Amplify or on its own) can be used as a third-party authentication provider alongside Savira Auth, or standalone, with your Savira project.

## Getting started

1. First you need to add an integration to connect your Savira project with your Amazon Cognito User Pool. You will need the pool's ID and region.
...
By default, Amazon Cognito JWTs (both ID token and access tokens) do not contain a `role` claim in them. If you were to send such a JWT to your Savira project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
