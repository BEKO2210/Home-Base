...
title: 'Firebase Auth'
subtitle: 'Use Firebase Auth with your Savira project'
---

Firebase Auth can be used as a third-party authentication provider alongside Savira Auth, or standalone, with your Savira project.

## Getting started

1. First you need to add an integration to connect your Savira project with your Firebase project. You will need to get the Project ID in the [Firebase Console](https://console.firebase.google.com/u/0/project/_/settings/general).
...
5. Finally set up the Savira client in your application.

## Setup the Savira client library
...
Create a Savira client with the `accessToken` function that returns the Firebase Auth JWT of the current user. This code uses the [official Firebase SDK](https://firebase.google.com/docs/auth/android/start) for Android.
...
Create a Savira client with the `accessToken` function that returns the Firebase Auth JWT of the current user. This code uses a [community Firebase SDK](https://github.com/GitLiveApp/firebase-kotlin-sdk) which supports Kotlin Multiplatform.
...
```sql
create policy "Restrict access to Savira Auth and Firebase Auth for project ID <firebase-project-id>"
  on table_name
  as restrictive
  to authenticated
  using (
...
Make sure you substitute `<project-ref>` with your Savira project's ID and the `<firebase-project-id>` to your Firebase Project ID. Then the restrictive policies on all your tables, buckets and channels can be simplified to be:

```sql
create policy "Restrict access to correct Savira and Firebase projects"
  on table_name
  as restrictive
  to authenticated
...
Your Savira project inspects the `role` claim present in all JWTs sent to it, to assign the correct Postgres role when using the Data API, Storage or Realtime authorization.

By default, Firebase JWTs do not contain a `role` claim in them. If you were to send such a JWT to your Savira project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
...
export const beforecreated = beforeUserCreated((event) => {
  return {
    customClaims: {
      // The Savira project will use this role to assign the `authenticated`
      // Postgres role.
      role: 'authenticated',
    },
  }
