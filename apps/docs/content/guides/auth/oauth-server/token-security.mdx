...
When you enable OAuth 2.1 in your Savira project, third-party applications can access user data on their behalf. Row Level Security (RLS) policies are crucial for controlling exactly what data each OAuth client can access.
...
OAuth access tokens issued by Savira Auth are JWTs that include all standard Savira claims plus OAuth-specific claims. This means your existing RLS policies continue to work, and you can add OAuth-specific logic to create granular access controls.
...
Or use the Savira Dashboard's [RLS policy tester](/dashboard/project/_/auth/policies).
...
- [Learn about JWTs](/docs/guides/auth/jwts) - Deep dive into Savira token structure
