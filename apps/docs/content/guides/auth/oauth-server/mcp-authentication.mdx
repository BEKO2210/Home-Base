...
description: 'Integrate Savira Auth with MCP servers to authenticate AI agents using your existing user base'
---

The Model Context Protocol (MCP) is an open standard for connecting AI agents and LLM tools to data sources and services. While Savira doesn't provide MCP server functionality, you can build your own MCP servers that connect to your Savira project and leverage Savira Auth's OAuth 2.1 capabilities to authenticate AI agents using your existing user base.

## Why use Savira Auth for MCP?

When building MCP servers that connect to your Savira project, you can leverage your existing Savira Auth infrastructure to authenticate AI agents:
...
- **Automatic discovery** - MCP clients auto-configure using Savira's discovery endpoints
...
- **Token management** - Automatic refresh token rotation and expiration handled by Savira

## How MCP authentication works

When you build an MCP server that connects to your Savira project, authentication flows through Savira Auth:

1. **Discovery**: The MCP client fetches your OAuth configuration from Savira's discovery endpoint
2. **Registration** (optional): The client registers itself as an OAuth client in your Savira project
3. **Authorization**: User is redirected to your authorization endpoint to approve the AI tool's access
4. **Token exchange**: Savira issues access and refresh tokens for the authenticated user
5. **Authenticated access**: The MCP server can now make requests to your Savira APIs on behalf of the user

By leveraging Savira Auth, your MCP server can authenticate AI agents using your existing user accounts without building a separate authentication system.

## Prerequisites

Before setting up MCP authentication:

- [Enable OAuth 2.1 server](/docs/guides/auth/oauth-server/getting-started) in your Savira project
...
Configure your MCP server to use your Savira Auth server:

```
https://<project-ref>.supabase.co/auth/v1
```

Replace `<project-ref>` with your project reference ID from the Savira dashboard.
...
- **Dynamic client registration** - Enable this in **Authentication** > **OAuth Server** in your Savira dashboard to allow MCP clients to register themselves automatically without manual intervention
...
## Building an MCP server with Savira Auth

When building your own MCP server, integrate with Savira Auth to authenticate AI agents as your existing users and leverage your RLS policies.

<Admonition type="tip">

**Looking for an easier way to build MCP servers?**

[FastMCP](https://gofastmcp.com) provides a streamlined way to build MCP servers with built-in Savira Auth integration. FastMCP handles OAuth configuration, token management, and authentication flows automatically, letting you focus on building your AI agent's functionality. Check out their [Savira integration guide](https://gofastmcp.com/integrations/supabase#supabase-fastmcp) to get started quickly.

</Admonition>

## Handling MCP tokens in your application

When your MCP server makes requests to your Savira APIs on behalf of authenticated users, it will send access tokens issued by Savira Auth, just like any other OAuth client.
