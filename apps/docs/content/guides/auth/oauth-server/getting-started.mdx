...
description: 'Learn how to enable OAuth 2.1 and register client applications in your Savira project'
---

This guide will walk you through setting up your Savira project as an OAuth 2.1 identity provider, from enabling the feature to registering your first client application.

## Prerequisites

Before you begin, make sure you have:

- A Savira project (create one at [supabase.com](https://supabase.com))
- Admin access to your project
- (Optional) [Savira CLI](/docs/guides/cli) v2.54.11 or higher for local development

## Overview

Setting up OAuth 2.1 in your Savira project involves these steps:
...
Testing OAuth flows is often easier on a Savira project since it's already accessible on the web, no tunnel or additional configuration needed.

</Admonition>

## Enable OAuth 2.1 server

OAuth 2.1 server is currently in beta and free to use during the beta period on all Savira plans.
...
Start or restart your local Savira instance:
...
To test OAuth flows with external applications, you can expose your local Savira instance using a tunnel solution (such as [ngrok](https://ngrok.com/) or [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/)).
...
By default, Savira uses HS256 (symmetric) for signing JWTs. For OAuth use cases, we recommend migrating to asymmetric algorithms like RS256 or ES256. Asymmetric keys are more scalable and secure because:
...
When OAuth clients initiate the authorization flow, Savira Auth will redirect users to this URL with an `authorization_id` query parameter. You'll use [Savira JavaScript library OAuth methods](https://github.com/supabase/supabase-js/blob/master/packages/core/auth-js/src/GoTrueClient.ts#L2159-L2163) to handle the authorization:
...
This is a **frontend implementation**. You're building the UI that displays the consent screen and handles user interactions. The actual OAuth token generation is handled by Savira Auth after you call the approve/deny methods.
...
1. **User navigates to your authorization path** - When a third-party app initiates OAuth, Savira Auth redirects the user to your configured authorization path (e.g., `https://example.com/oauth/consent?authorization_id=<id>`)
...
Redirect URIs are critical for OAuth security. Savira Auth will only redirect to URIs that are explicitly registered with the client.

<Admonition type="note">

**Not to be confused with general redirect URLs**

This section is about **OAuth client redirect URIs** - where to send users after they authorize third-party apps to access your Savira project. This is different from the general [Redirect URLs](/docs/guides/auth/redirect-urls) setting, which controls where to send users after they sign in TO your app using social providers.
