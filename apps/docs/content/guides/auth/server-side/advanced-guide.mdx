...
When a user authenticates with Savira Auth, two pieces of information are issued by the server:
...
To maintain the session, these tokens must be stored in a storage medium securely shared between client and server, which is traditionally cookies. Whenever the session is refreshed, the auth and refresh tokens in the shared storage medium must be updated. Savira client libraries provide a customizable `storage` option when a client is initiated, allowing you to change where tokens are stored.
...
If you use ISR on pages that trigger a Savira session refresh, the cached response will include the `Set-Cookie` header containing the refreshed JWT. When that cached response is served to a subsequent user, their browser stores the token and they are signed in as the wrong person.
...
Vercel's Fluid compute model can keep server instances warm and reuse them across requests. In some cases this means a Savira client initialized in module scope — or stored in a shared variable — may be reused across requests from different users, causing one user's session to leak into another user's request.

Always initialize the Savira client inside the request handler, not at module level. Do not store the client or any user-specific state in a variable that persists between requests.

### Which authentication flows have PKCE support?

At present, PKCE is supported on the Magic Link, OAuth, Sign Up, and Password Recovery routes. These correspond to the `signInWithOtp`, `signInWithOAuth`, `signUp`, and `resetPasswordForEmail` methods on the Savira client library. When using PKCE with Phone and Email OTPs, there is no behavior change with respect to the implicit flow - an access token will be returned in the body when a request is successful.
