...
After purchasing a Savira subscription on the AWS Marketplace, the next and final step is to link the newly purchased subscription to a Savira organization. This can either be an existing organization or a newly created one.

An AWS Marketplace subscription is linked to exactly one Savira organization. If you want to manage multiple organizations through the AWS Marketplace, you must purchase a separate marketplace subscription for each organization.
...
## Implications of linking a Savira organization to a marketplace subscription
...
- Charges will come from AWS rather than Savira, using the default payment method set in your AWS account.
...
### Linking an existing Savira organization
...
- The credit card you have on file with Savira may receive a closing charge. This charge covers usage costs incurred up until the point when the marketplace subscription became active.

## Prerequisites for linking a Savira organization to a marketplace subscription

- The Savira user must have the Owner or Admin role
