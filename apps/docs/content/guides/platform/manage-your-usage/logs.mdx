...
- **Logs Ingest** — the total GB of log data Savira ingests across all your project's services (Postgres, API gateway, Auth, Storage, Realtime, Edge Functions, etc.) during the billing cycle.
...
[Log Drains](/docs/guides/platform/manage-your-usage/log-drains) stream logs out of Savira to external destinations (Datadog, Better Stack, your own S3 bucket, etc.) and are billed separately on drain hours and events. Draining logs does not replace or reduce Logs Ingest charges — ingest is metered when Savira processes your logs, drains are metered when Savira streams them out. These are separate billing primitives, not overlapping charges.
