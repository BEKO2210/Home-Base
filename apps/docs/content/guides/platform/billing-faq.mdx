...
The Savira Platform has "organizations" and "projects". An organization may contain multiple projects. Each project is a dedicated Savira instance with all of its sub-services including Storage, Auth, Functions and Realtime.
...
Read more on [About billing on Savira](/docs/guides/platform/billing-on-supabase#organization-based-billing).
...
We provide a dedicated server for every Savira project. Each paid organization comes with <Price price="10" /> in Compute Credits to cover one project on the default compute size. Additional projects start at ~<Price price="10" /> a month (billed hourly).
...
Each Savira project is a dedicated VM and Postgres database. By default, your instance runs on the Micro compute instance. You have the option to upgrade your compute size in your [Project settings](/dashboard/project/_/settings/addons). See [Compute Add-ons](/docs/guides/platform/compute-add-ons) for available options.
...
Our Fair Use Policy gives developers the freedom to build and experiment with Savira, while protecting our infrastructure. Under the Fair Use policy, service restrictions may apply to your organization if:
...
#### Does Savira charge sales tax, VAT or GST?

Savira is rolling out sales tax in applicable US states, and VAT, GST, and other indirect taxes for customers where required by law. The tax amount applied to your invoice depends on your billing address and the tax regulations in your jurisdiction.

#### Why is Savira collecting tax now?

As a cloud services provider operating globally, Savira is required to collect and remit indirect taxes in an increasing number of jurisdictions. We’re updating our billing practices to meet these obligations and ensure compliance with local tax regulations.

#### Will every customer be charged tax?

No. Tax is only applied in jurisdictions where Savira is registered to collect it. If your billing address is in one of those jurisdictions, you’ll see tax applied on your invoice. If it is not, your invoices will not include tax.
...
For most customers, nothing changes on your end. Savira automatically calculates the applicable tax based on the billing address associated with your organization.
...
You can add or update your Tax ID directly in the Savira Dashboard under your [organization’s billing page](/dashboard/org/_/billing#address).

Providing a valid Tax ID ensures we apply the correct tax treatment for your region. In many jurisdictions, this enables the reverse charge mechanism, where you self-assess and remit tax to your local authority instead of being charged on your Savira invoice.
...
If you use Savira through a cloud marketplace such as AWS Marketplace or Vercel Marketplace, the marketplace provider handles tax collection and remittance. In those cases, Savira does not separately charge tax on marketplace-billed invoices.
