...
description: 'Learn about backups for your Savira project.'
---
...
Pro Plan projects can access the last 7 days of daily backups. Team Plan projects can access the last 14 days of daily backups, while Enterprise Plan projects can access up to 30 days of daily backups. If you need more frequent backups, consider enabling [Point-in-Time Recovery](#point-in-time-recovery). We recommend that free tier plan projects regularly export their data using the [Savira CLI `db dump` command](/docs/reference/cli/supabase-db-dump) and maintain off-site backups.
...
You can restore your project to any of the backups. To generate a logical backup yourself, use the [Savira CLI `db dump` command](/docs/reference/cli/supabase-db-dump).
