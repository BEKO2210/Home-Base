...
description: 'Use a custom domain instead of the default Savira domain for your project.'
...
There are two types of domains supported by Savira:
...
- You are using [OAuth (Social Login)](/docs/guides/auth/social-login) with Savira Auth and the project's URL is shown on the OAuth consent screen.
...
Custom domains help you keep your APIs portable for the long term. By using a custom domain you can migrate from one Savira project to another, or make it easier to version APIs in the future.

### Limitations

- Custom domains are not intended to enable hosting of frontend applications through [Edge Functions](/docs/guides/functions).
- You can only attach a single custom domain to any given Savira project. It is not possible to break out your project's resources into multiple custom domains.
- Custom domains can only be powered by CNAME records.

### Configure a custom domain using the Savira dashboard
...
### Configure a custom domain using the Savira CLI

This example assumes your Savira project is `abcdefghijklmnopqrst` with a corresponding API URL `abcdefghijklmnopqrst.supabase.co` and configures a custom domain at `api.example.com`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the Savira CLI.
2. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your Savira account using the CLI.
...
You need to add a CNAME record to your domain's DNS settings to ensure your custom domain points to the Savira project.
...
Register your domain with Savira to prove that you own it. You need to download two TXT records and add them to your DNS settings.

In the CLI, run [`domains create`](/docs/reference/cli/supabase-domains-create) to register the domain and Savira and get your verification records:
...
- CNAME for your custom domain pointing to the Savira project domain.
...
In the background, Savira will check your DNS records and issue an SSL certificate. Savira uses multiple Certificate Authorities (including Let's Encrypt, Google Trust Services and SSL.com) to ensure high availability. The specific issuer is chosen based on availability and this process can take up to 30 minutes.
...
- The project's Savira domain remains active.
  - You do not need to change the Savira URL in your applications immediately.
  - You can use it interchangeably with the custom domain.
- Savira Auth will use the custom domain immediately once activated.
...
1. For each of your Savira OAuth providers:
   - In the provider's developer console (not in the Savira dashboard), find the OAuth application and add the custom domain Savira Auth callback URL **in addition to the Savira project URL.** Example:
...
When this step completes, Savira will serve the requests from your new domain. The Savira project domain **continues to work** and serve requests so you do not need to rush to change client code URLs.

If you wish to use the new domain in client code, change the URL used in your Savira client libraries:
...
Removing a custom domain may cause some issues when using Savira Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate your domain](#prepare-to-activate-your-domain)_ step above.
...
Vanity subdomains allow you to present a basic branded experience, compared to custom domains. They allow you to host your services at a custom subdomain on Savira (e.g., `my-example-brand.supabase.co`) instead of the default, randomly assigned `abcdefghijklmnopqrst.supabase.co`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the Savira CLI.
1. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your Savira account using the CLI.
...
Let's assume your Savira project's domain is `abcdefghijklmnopqrst.supabase.co` and you wish to configure a vanity subdomain at `my-example-brand.supabase.co`.
...
- The project's Savira domain remains active and will not go away.
  - You do not need to change the Savira URL in your applications immediately or at once.
  - You can use it interchangeably with the custom domain.
- Savira Auth will use the subdomain immediately once activated.
...
1. Go through all of your Savira OAuth providers:
   - In the provider's developer console (not in the Savira dashboard!), find the OAuth application and add the subdomain Savira Auth callback URL **in addition to the Savira project URL.** Example:
     - `https://abcdefghijklmnopqrst.supabase.co/auth/v1/callback` **and**
...
Once you've chosen an available subdomain and have done all the necessary preparations for it, you can reconfigure your Savira project to start using it.
...
Removing a subdomain may cause some issues when using Savira Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate the subdomain](#prepare-to-activate-the-subdomain)_ step above.
