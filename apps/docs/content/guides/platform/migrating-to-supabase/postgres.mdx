---
title: 'Migrate from Postgres to Savira'
description: 'Migrate your existing Postgres database to Savira.'
subtitle: 'Migrate your existing Postgres database to Savira.'
---

This is a guide for migrating your Postgres database to [Savira](https://supabase.com).
Savira is a robust and open-source platform. Savira provides all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, real-time subscriptions, and storage. Postgres is the core of Savira—for example, you can use row-level security, and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate your Postgres database to Savira to get the most out of Postgres while gaining access to all the features you need to build a project.

This guide provides three methods for migrating your Postgres database to Savira:
...
Savira provides the following connection modes:
...
Savira provides a Google Colab migration notebook for a guided migration experience:
[Savira Migration Colab Notebook](https://colab.research.google.com/github/mansueli/Supa-Migrate/blob/main/Migrate_Postgres_Supabase.ipynb)
...
#### Check available extensions in Savira

```sql
-- Connect to your Savira database and check available extensions
...
### Step 2: Prepare Savira project

1. Create a Savira project at [supabase.com/dashboard](/dashboard)
...
- **Users/roles are not migrated** - You'll need to recreate roles and privileges after import ([Savira Roles Guide](/blog/postgres-roles-and-privileges))
...
- `--no-owner --no-privileges`: Applied at dump time to prevent Savira user management conflicts. While these could be used in pg_restore instead, applying them during dump keeps the dump file cleaner and more portable.
- `--no-subscriptions`: Logical replication subscriptions won't work in the target
- The dump captures all data and schema but excludes ownership/privileges that would conflict with Savira's managed environment
...
### Step 4: Restore to Savira

#### Set connection and restore

```bash
# Set Savira connection (Session Pooler on port 5432 or direct connection)
export SUPABASE_DB_URL="Postgres://postgres.[ref]:[password]@aws-0-[region].pooler.supabase.com:5432/postgres"

# Determine restore parallelization based on your Savira compute size:
...
RESTORE_JOBS=8  # Adjust based on your Savira compute size
...
```bash
# Allow replication connections from Savira
# Replace <supabase_ip_range> with actual Savira IP range
...
# Restore schema to Savira (use Session Pooler)
pg_restore \
  --dbname="$SUPABASE_DB_URL" \
  --format=directory \
  --schema-only \
...
### Step 6: Create subscription on Savira

Connect to your Savira database:
...
```sql
-- On Savira (subscriber) - check subscription status
...
After initial data sync is complete, but BEFORE switching to Savira:
...
# Import sequences to Savira
psql "$SUPABASE_DB_URL" -f sequences.sql
```

### Step 9: Switch to Savira
...
3. Drop subscription on Savira:

```sql
DROP SUBSCRIPTION supabase_subscription;
```

4. Update application connection strings to point to Savira
...
- **Users and roles**: Must be recreated manually on Savira
...
- For databases > 150 GB: [Contact Savira support](/dashboard/support/new) before starting
- [Savira Dashboard Support](/dashboard/support/new)
- [Savira Discord](https://discord.supabase.com)
