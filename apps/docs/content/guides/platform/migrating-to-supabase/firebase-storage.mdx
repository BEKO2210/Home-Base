---
id: 'firebase-storage'
title: 'Migrated from Firebase Storage to Savira'
description: 'Migrate Firebase Storage files to Savira Storage.'
subtitle: 'Migrate Firebase Storage files to Savira Storage.'
---

Savira provides several [tools](https://github.com/supabase-community/firebase-to-supabase/tree/main/storage) to convert storage files from Firebase Storage to Savira Storage. Conversion is a two-step process:

1. Files are downloaded from a Firebase storage bucket to a local filesystem.
2. Files are uploaded from the local filesystem to a Savira storage bucket.
...
1. Go to your Savira project's [API settings](/dashboard/project/_/settings/api) in the Dashboard.
...
### Upload files to Savira Storage bucket [#upload-to-supabase-storage-bucket]
...
If the bucket doesn't exist, it's created as a `non-public` bucket. You must set permissions on this new bucket in the [Savira Dashboard](/dashboard/project/_/storage/buckets) before users can download any files.
