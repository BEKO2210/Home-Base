...
description: 'Enable temporary access for short-lived database connections tied to a Savira user.'
---

Your Savira project supports connecting to the Postgres database using either your Savira API token (Personal Access Token) or your current dashboard session token (JWT). This is called temporary access, as the authentication tokens can be short-lived and tied directly to a specific Savira user. Temporary access is disabled by default.

Enabling temporary access only applies to connections to Postgres and Supavisor ("Connection Pooler"); all HTTP APIs offered by Savira (e.g., PostgREST, Storage, Auth) require authentication tokens specific to the service and are independent of the Savira platform user(s).
...
Temporary access authentication only applies for Savira project users authenticating to the database, and assuming the given Postgres role. No new roles or users are created in the Postgres database and the assumed role's permissions will still apply.
...
Since Savira API tokens can be used, it is also possible to generate API tokens for services you don't want to share your Postgres role password with (for example a GitHub Action). The API token can be configured with an expiry time and temporary access-specific restrictions can also be applied.
