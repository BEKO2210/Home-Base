...
1. **Access from identity provider:**
   ...
   - Locate your Savira app tile or bookmark
   - Click the app tile

2. **Verify authentication:**
   - If already authenticated with IdP: Immediate redirect to Savira
...
3. **Verify redirect chain:**
   ...
   - Should redirect back to Savira
...
- ✅ Successful authentication redirects back to Savira
...
1. **Navigate to the SSO sign-in page**:
   - Visit [Sign in with SSO](/dashboard/sign-in-sso)
   - Or click "Sign in with SSO" from the main Savira sign-in page
...
4. **Confirm successful sign-in**:
   - You should be redirected back to Savira dashboard
...
3. **Test auto-join with your account**:
   - **Log out completely** from Savira
...
- User not assigned to Savira app in identity provider
- Email domain not configured in Savira SSO settings
...
5. Check domain configuration in Savira matches user email
...
- Update metadata in Savira after IdP certificate renewal
...
- Keep Savira support contact information handy
...
- User access reviews should include Savira
...
If you encounter issues not covered in this guide, contact your Savira support representative for assistance. When reaching out, include:
