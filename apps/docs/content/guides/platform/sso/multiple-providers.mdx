...
Many enterprises need multiple single sign-on (SSO) providers configured within Savira to support different environments, teams, or organizational structures. This guide explains when and how to set up multiple providers effectively.
...
You have three Savira organizations:
...
Create three separate SAML apps in your identity provider, each pointing to a different Savira organization.

#### In your identity provider (Okta, Azure AD, Google Workspace)

1. **Create "Savira Dev" SAML app**
   - Configure with Dev organization's ACS URL
   - Assign developers and testers
   - Deploy app tile labeled "Savira Dev"

2. **Create "Savira Staging" SAML app**
   - Configure with Staging organization's ACS URL
   - Assign QA team and release managers
   - Deploy app tile labeled "Savira Staging"

3. **Create "Savira Production" SAML app**
   - Configure with Production organization's ACS URL
   - Assign production users only
   - Deploy app tile labeled "Savira Production"

#### In each Savira organization
...
| ----------- | ------------ | ------------------ | ------------------------------------ |
| Dev         | `dev`        | "Savira Dev"     | `https://...dev-org.../saml/acs`     |
| Staging     | `staging`    | "Savira Staging" | `https://...staging-org.../saml/acs` |
| Production  | `prod`       | "Savira Prod"    | `https://...prod-org.../saml/acs`    |
...
Some organizations need to isolate teams within separate Savira organizations.
...
- "Savira Engineering" → Points to `engineering-org`
- "Savira Data" → Points to `data-org`
...
#### Step 3: Configure each Savira organization
...
3. Verify redirect to correct Savira organization
...
Engineering Team:
├─ Savira Dev (assigned) ✅
├─ Savira Staging (assigned) ✅
└─ Savira Prod (assigned) ✅

QA Team:
├─ Savira Dev (assigned) ✅
├─ Savira Staging (assigned) ✅
└─ Savira Prod (NOT assigned) ❌

Contractors:
├─ Savira Dev (assigned) ✅
├─ Savira Staging (NOT assigned) ❌
└─ Savira Prod (NOT assigned) ❌
```
...
#### Savira organization names

Match your IDP app names when possible:

- IDP app: "Savira - Production" → Org: `acme-production`
- IDP app: "Savira - Staging" → Org: `acme-staging`
- IDP app: "Savira - Dev" → Org: `acme-development`
...
User clicks "Savira Prod" tile but sees the dev environment.
...
2. Compare metadata in Savira SSO settings
...
User cannot find "Savira Staging" tile in IDP dashboard.
...
1. Create additional Savira organizations
