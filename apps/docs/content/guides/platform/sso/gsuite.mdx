...
Savira supports single sign-on (SSO) using Google Workspace (formerly known as G Suite).
...
The information you enter here is for visibility into your Google Workspace. You can choose any values you like. `Savira` as a name works well for most use cases. Optionally enter a description.
...
**Certificate expiration:** Set a calendar reminder 30 days before the certificate expiration date. When the certificate is renewed, you'll need to download the new metadata file and update it in your Savira SSO settings. Expired certificates are a common cause of SSO sign-in failures.
...
| Start URL      | `https://savira.io/dashboard`                    |
...
Attribute mappings allow Savira to get information about your Google Workspace users on each login.
...
You can configure which Google Workspace user accounts will get access to Savira. This is important if you wish to limit access to your software engineering teams.

You can configure this access by clicking on the _User access_ card (or down-arrow). Follow the instructions on screen.

![Google Workspace: Web and mobile apps admin console, Savira app screen](/docs/img/sso-gsuite-step-08.png)
