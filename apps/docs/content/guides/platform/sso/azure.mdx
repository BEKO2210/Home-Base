...
Looking for docs on how to add Single Sign-On support in your Savira project? Head on over to [Single Sign-On with SAML 2.0 for Projects](/docs/guides/auth/enterprise-sso/auth-sso-saml).

</Admonition>

Savira supports single sign-on (SSO) using Microsoft Azure AD.
...
You'll be using the custom enterprise application setup for Savira.
...
In the modal titled _Create your own application_, enter a display name for Savira. This is the name your Azure AD users will see when signing in to Savira from Azure. `Savira` works in most cases.
...
Before you get to assigning users and groups, which would allow accounts in Azure AD to access Savira, you need to configure the SAML details that allows Savira to accept sign in requests from Azure AD.

![Azure AD console: Savira custom enterprise application, selected Set up single sign-on](/docs/img/sso-azure-step-04.png)

## Step 5: Select SAML single sign-on method [#saml-sso]

Savira only supports the SAML 2.0 protocol for Single Sign-On, which is an industry standard.

![Azure AD console: Savira application, Single sign-on configuration screen, selected SAML](/docs/img/sso-azure-step-05.png)

## Step 6: Upload SAML-based sign-on metadata file [#upload-saml-metadata]

First you need to download Savira's SAML metadata file. Click the button below to initiate a download of the file.

    <a href="https://alt.supabase.io/auth/v1/sso/saml/metadata?download=true">
      <Button size="large" icon={<IconArrowDown />}>
        Download Savira SAML Metadata File
      </Button>
    </a>
...
![Azure AD console: Savira application, SAML-based Sign-on screen, selected Upload metadata file button](/docs/img/sso-azure-step-06-1.png)

All of the correct information should automatically populate the _Basic SAML Configuration_ screen as shown.

![Azure AD console: Savira application, SAML-based Sign-on screen, Basic SAML Configuration shown](/docs/img/sso-azure-step-06-2.png)

**Make sure you input these additional settings.**

| Setting     | Value                                        |
| ----------- | -------------------------------------------- |
| Sign on URL | `https://savira.io/dashboard/sign-in-sso` |
| Relay State | `https://savira.io/dashboard`             |
...
![Azure AD console: Savira application, SAML Certificates card shown, App Federation Metadata Url highlighted](/docs/img/sso-azure-step-07.png)
