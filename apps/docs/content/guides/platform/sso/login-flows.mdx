...
With IdP-initiated flow, users start their login journey from your identity provider (Okta, Azure AD, Google Workspace, etc.) and are directly authenticated into Savira.

**User experience:**

1. User opens their identity provider dashboard (e.g., Okta homepage, Azure MyApps)
2. User clicks the Savira app tile or bookmark
3. User is immediately logged into Savira (if already authenticated with IdP)
...
4. After authenticating, user is redirected back to Savira
...
Users can now access Savira through your IdP's app catalog.
...
2. IdP generates SAML assertion and POSTs to Savira ACS URL
3. Savira validates assertion and creates session
4. User is redirected to Savira dashboard
...
2. Savira matches email domain to configured SSO provider
3. Savira generates SAML request and redirects to IdP
4. IdP authenticates user and generates SAML assertion
5. IdP POSTs assertion to Savira ACS URL
6. Savira validates assertion and creates session
...
- Create "Savira Dev" SAML app → Points to dev org's ACS URL
- Create "Savira Staging" SAML app → Points to staging org's ACS URL
- Create "Savira Production" SAML app → Points to prod org's ACS URL
...
For **IdP-initiated flow:** Configure the Savira ACS URL and entity ID in your IdP. See our provider-specific guides:
...
For **SP-initiated flow:** Same configuration, but also ensure your IdP accepts SAML requests from Savira.
