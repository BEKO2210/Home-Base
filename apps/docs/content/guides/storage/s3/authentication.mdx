...
subtitle: 'Learn about authenticating with Savira Storage S3.'
sidebar_label: 'S3'
---

You have two options to authenticate with Savira Storage S3:
...
This is all the information you need to connect to Savira Storage using any S3-compatible service.
...
You can authenticate to Savira S3 with a user JWT token to provide limited access via RLS to all S3 operations. This is useful when you want initialize the S3 client on the server scoped to a specific user, or use the S3 client directly from the client side.
...
- On self-hosted Savira, the `accessKeyId` is the `STORAGE_TENANT_ID` environment variable defined in the `.env` file. Refer to the [self-hosted S3 guide](/docs/guides/self-hosting/self-hosted-s3#session-token) for more details.
