...
Analytics Buckets are still available, but managed replication into Analytics Buckets through Savira ETL is no longer supported. If you need managed replication today, use [external replication](/docs/guides/database/replication/external-replication-setup) with **BigQuery**. If you want to use Analytics Buckets, bring your own ingestion pipeline.

</Admonition>

## Creating an Analytics bucket

You can create an analytics bucket using either the Savira SDK or the Savira Dashboard.

### Using the Savira SDK
...
### Using the Savira Dashboard

1. Navigate to the **Storage** section in the Savira Dashboard.
