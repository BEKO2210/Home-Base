...
description: 'How to manage schema migrations for your Savira project.'
subtitle: 'How to manage schema migrations for your Savira project.'
...
You will need to [install](/docs/guides/local-development#quickstart) the Savira CLI and start the local development stack.
...
You've been developing your project locally, making changes to your tables via migrations. It's time to deploy your project to the Savira Platform and start scaling up to millions of users!

Head over to [Savira](/dashboard) and create a new project to deploy to.

<StepHikeCompact>

  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Log in to the Savira CLI">
      [Login](/docs/reference/cli/supabase-login) to the Savira CLI using an auto-generated Personal Access Token.
    </StepHikeCompact.Details>
...
Visiting your live project on [Savira](/dashboard/project/_), you'll see a new `employees` table, complete with the `department` column you added in the second migration above.

## Working with a team

When multiple developers share a Savira project, a few rules keep migrations from getting out of sync.
...
For a more automated deployment approach, consider using [Savira Branching](/docs/guides/deployment/branching) or a CI/CD pipeline that runs `supabase db push` on merge to your main branch.
...
Savira tracks which migrations have been applied on each database in a table called `supabase_migrations.schema_migrations`. When you run `supabase db push`, it compares your local `supabase/migrations` folder against that table and runs only the ones not yet applied, in order.

Git tracks your migration _files_. Savira tracks what's been _applied to each database_. These are two separate systems that need to stay in sync.
