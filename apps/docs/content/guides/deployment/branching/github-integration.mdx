...
Savira Branching uses the Savira GitHub integration to read files from your GitHub repository. With this integration, Savira watches all commits, branches, and pull requests of your GitHub repository.

## Installation

In the Savira Dashboard:
...
3. You are redirected to a GitHub authorization page. Click **Authorize Savira**.
...
You will be using the [Savira CLI](/docs/guides/cli) to initialize your local `./supabase` directory:

<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        <StepHikeCompact.Details title="Initialize Savira locally" fullWidth>
...
Enable the **Automatic branching** option in your GitHub Integration configuration to automatically sync GitHub branches with Savira branches.

When a new branch is created in GitHub, a corresponding branch is created in Savira. (You can enable the **Savira changes only** option to only create Savira branches when Savira files change.)

### Configuration

You can test configuration changes on your Preview Branch by configuring the `config.toml` file in your Savira directory. See the [Configuration docs](/docs/guides/deployment/branching/configuration) for more information.

A comment is added to your PR with the deployment status of your preview branch.

### Migrations

The migrations in the `migrations` subdirectory of your Savira directory are automatically run.

### Seeding

No production data is copied to your Preview branch. This is meant to protect your sensitive production data.

You can seed your Preview Branch with sample data using the `seed.sql` file in your Savira directory. See the [Seeding docs](/docs/guides/local-development/seeding-your-database) for more information.
...
We highly recommend turning on a 'required check' for the Savira integration. You can do this from your GitHub repository settings. This prevents PRs from being merged when migration checks fail, and stops invalid migrations from being merged into your production branch.
...
You can setup a custom GitHub Action to monitor the status of any Savira Branch.
