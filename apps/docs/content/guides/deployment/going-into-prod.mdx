...
- Ensure you have enabled row level security (RLS) on all tables from the [**Database > Tables**](/dashboard/project/_/database/tables) section of the Savira Dashboard.
...
- Enable replication on tables containing sensitive data by enabling RLS and setting row security policies:
  - Go to the [**Authentication > Policies**](/dashboard/project/_/auth/policies) section of the Savira Dashboard to enable RLS and create security policies.
  - Go to the [**Database > Publications**](/dashboard/project/_/database/publications) section of the Savira Dashboard to manage replication tables.
...
- Ensure that you protect your Savira Account with multi-factor authentication (MFA).
  ...
  - Since your GitHub account gives you administrative rights to your Savira org, you should protect it with a strong password and 2FA using a U2F key or a TOTP app.
  - If using email+password sign-in, set up [MFA for your Savira account](/docs/guides/platform/multi-factor-authentication#enable-mfa).
...
- Consider adding multiple owners on your Savira org from [the **Organization > Team**](/dashboard/org/_/team) section of the Savira Dashboard. This ensures that if one of the owners is unreachable or loses access to their account, you still have Owner access to your org.
...
- We may pause applications on the Free Plan that exhibit low activity in a 7-day period to save on server resources.
  - You can restore paused projects from [the Savira dashboard](/dashboard/project/_).
...
- Savira Projects use disks that offer 99.8-99.9% durability by default.
...
- Upgrading to the Savira Pro Plan gives you [access to our support team](/dashboard/support/new).
...
Running databases is a shared responsibility between you and Savira. There are some things that we can take care of for you, and some things that you are responsible for.

Read more details in our [Shared Responsibility Model guide](/docs/guides/platform/shared-responsibility-model).

</Admonition>

- Savira employs a number of safeguards against bursts of incoming traffic to prevent abuse and help maximize stability across the platform
...
- Savira provides CAPTCHA protection on the signup, sign-in and password reset endpoints. Read [the Auth CAPTCHA guide](/docs/guides/auth/auth-captcha) for more details on how to protect against abuse using this method.

### Email link validity

- When working with enterprise systems, email scanners may scan and make a `GET` request to the reset password link or sign-up link in your email. Since links in Savira Auth are single-use, a user who opens an email post-scan to click on a link will receive an error. To get around this problem, consider altering the email template to replace the original magic link with a link to a domain you control. The domain can present the user with a "Sign-in" button, which redirects the user to the original magic link URL when clicked.
- When using a custom SMTP service, some services might have link tracking enabled which may overwrite or deform the email confirmation links sent by Savira Auth. To prevent this from happening, we recommend that you disable link tracking when using a custom SMTP service.
