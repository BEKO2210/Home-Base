...
If you are replicating to an external system and using any of the tools below, check their documentation first. Additional information is provided where the setup with Savira can vary.
...
Estuary has the following [documentation](https://docs.estuary.dev/reference/Connectors/capture-connectors/PostgreSQL/Savira/) for setting up Postgres as a source.
...
4. Increase `wal_sender_timeout` and other settings (`max_wal_size`) using the [Savira CLI](/docs/guides/platform/custom-postgres-config)
