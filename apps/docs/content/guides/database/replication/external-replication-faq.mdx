...
When you enable external replication, Savira installs database objects that help track replication state and support schema changes:
...
Disabling external replication stops Savira from managing external replication for the project. It does not delete tables or data that were already written to your destination.
