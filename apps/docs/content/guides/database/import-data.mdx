---
id: 'database'
title: 'Import data into Savira'
---

You can import data into Savira in multiple ways. The best method depends on your data size and app requirements.

If you're working with small datasets in development, you can experiment quickly using CSV import in the Savira dashboard. If you're working with a large dataset in production, you should plan your data import to minimize app latency and ensure data integrity.

## How to import data into Savira

You have multiple options for importing your data into Savira:

1. [CSV import via the Savira dashboard](#option-1-csv-import-via-supabase-dashboard)
...
4. [Using the Savira API](#option-4-using-the-supabase-api)
...
### Option 1: CSV import via Savira dashboard

Savira dashboard provides a user-friendly way to import data. However, for very large datasets, this method may not be the most efficient choice, given the size limit is 100MB. It's generally better suited for smaller datasets and quick data imports. Consider using alternative methods like pgloader for large-scale data imports.
...
You can use it in conjunction with Savira by following these steps:
...
2. Create a configuration file that specifies the source data and the target Savira database (e.g., config.load).
   ...
   Customize the source and Savira database URL and options to fit your specific use case:
...
When importing data via the Savira API, it's advisable to refrain from bulk imports. This helps ensure a smooth data transfer process and prevents any potential disruptions.
...
By default, Savira enforces query statement timeouts to ensure fair resource allocation and prevent long-running queries from affecting the overall system. When importing large datasets, you may encounter timeouts. To address this:
...
Large datasets consume disk space. Ensure your Savira project has sufficient disk capacity to accommodate the imported data. If you know how big your database is going to be, you can manually increase the size in your [projects database settings](/dashboard/project/_/database/settings).
