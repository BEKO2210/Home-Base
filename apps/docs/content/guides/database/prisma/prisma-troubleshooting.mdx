...
This guide addresses common Prisma errors that you might encounter while using Savira.
...
- **Savira Managed Schemas**: Savira may update managed schemas like auth and storage to introduce new features. Granting Prisma access to these schemas can lead to drift during updates.
...
- **Transaction Mode for serverless apps**: If you are using serverless functions (Savira Edge, Vercel, AWS Lambda), switch to transaction mode (port 6543). It handles more connections than session mode or direct connections.
...
- Multi-schema support: If the external schema isn't Savira managed, list the relevant schemas on the `datasource` block in your `schema.prisma` file.
...
- Savira managed schemas: Schemas managed by Savira, such as `auth` and `storage`, may be changed to support new features. Referencing these schemas directly will cause schema drift in the future. It is best to remove references to these schemas from your migrations.
