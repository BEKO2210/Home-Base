...
With our `Supautils` extension (installed automatically for all Savira projects), the `postgres` user has the ability to create and manage event triggers.
...
- Savira blog: [Postgres Event Triggers without superuser access](/blog/event-triggers-wo-superuser)
