...
Your Postgres database is the core of your Savira project, so it's important that every role has a strong, secure password at all times. Here are some tips for creating a secure password:
...
Postgres comes with a set of [predefined roles](https://www.postgresql.org/docs/current/predefined-roles.html). Savira extends this with a default set of roles which are configured on your database when you start a new project:
...
Used by [Replication powered by Savira ETL](/docs/guides/database/replication) to replicate database changes to external destinations. Has read-all access and replication privileges for change data capture, bypasses Row Level Security, and can write to the `etl` schema.

### `dashboard_user`

For running commands via the Savira UI.
