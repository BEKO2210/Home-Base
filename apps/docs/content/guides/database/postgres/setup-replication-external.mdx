...
description: 'Example to setup logical replication using publish-subscribe to a Postgres database outside of Savira'
...
You will be running commands on both of these databases to publish changes from the Savira database to the external database.

1. Create a `publication` on the **Savira database**:

```sql
CREATE PUBLICATION example_pub;
```

2. Also on the **Savira database**, create a `replication slot`:
...
4. Now we'll go back to the Savira DB and add all the tables that you want replicated to the publication.
