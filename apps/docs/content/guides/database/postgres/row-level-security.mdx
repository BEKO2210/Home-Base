...
## Row Level Security in Savira

<Admonition type="danger">

Savira allows convenient and secure data access from the browser, as long as you enable RLS.
...
RLS is incredibly powerful and flexible, allowing you to write complex SQL rules that fit your unique business needs. RLS can be combined with [Savira Auth](/docs/guides/auth) for end-to-end user security from the browser to the database.
...
Using the `anon` Postgres role is different from an [anonymous user](/docs/guides/auth/auth-anonymous) in Savira Auth. An anonymous user assumes the `authenticated` role to access the database and can be differentiated from a permanent user by checking the `is_anonymous` claim in the JWT.
...
Savira provides some [helpers](#helper-functions) that simplify RLS if you're using Savira Auth. We'll use these helpers to illustrate some basic policies:
...
Savira provides special "Service" keys, which can be used to bypass RLS. These should never be used in the browser or exposed to customers, but they are useful for administrative tasks.

<Admonition type="note">

Savira will adhere to the RLS policy of the signed-in user, even if the client library is initialized with a Service Key.
