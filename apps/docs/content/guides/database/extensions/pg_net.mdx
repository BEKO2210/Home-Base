...
The data can also be observed in the `net` schema with the [Savira Dashboard's SQL Editor](/dashboard/project/_/editor)
...
begin
    ...
    -- Optionally Log row data or other data for inspection in Savira Dashboard's Postgres Logs
...
Savira supports reconfiguring pg*net starting from v0.12.0+. For the latest release, initiate a Postgres upgrade in the [Infrastructure Settings](/dashboard/project/*/settings/infrastructure).
...
### Invoke a Savira Edge Function

Make a POST request to a Savira Edge Function with auth header and JSON body payload:
