...
pg_repack requires the Postgres superuser role by default. That role is not available to users on the Savira platform. To avoid that requirement, use the `-k` or `--no-superuser-check` flags on every `pg_repack` CLI command.

The first version of pg_repack with full support for non-superuser repacking is 1.5.2. You can check the version installed on your Savira instance using
...
If pg_repack is not present, or the version is < 1.5.2, [upgrade to the latest version](/docs/guides/platform/upgrading) of Savira to gain access.

## Usage

### Enable the extension

Get started with pg_repack by enabling the extension in the Savira Dashboard.
