...
It's possible to connect to multiple remote servers of the same type. For example, you can connect to two different Firebase projects within the same Savira database.
...
Once the preceding steps are finished, the function can be invoked from Savira client to query the foreign table:
