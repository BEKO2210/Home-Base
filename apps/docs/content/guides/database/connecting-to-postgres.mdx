...
subtitle: 'Savira provides multiple methods to connect to your Postgres database, whether you’re working on the frontend, backend, or utilizing serverless functions.'
---
...
For convenience, you can also use the [Savira client libraries](/docs/reference), which wrap the Data APIs with a developer-friendly interface and automatically handle authentication:
...
Savira offers two poolers. The **Shared Pooler** ([Supavisor](https://github.com/supabase/supavisor)) is multi-tenant, available on every project, and IPv4-only. The **Dedicated Pooler** ([PgBouncer](https://www.pgbouncer.org/)) is available on paid plans and co-located with your Postgres instance; like the direct connection, it is on IPv6, or on IPv4 if the project has the [IPv4 add-on](/docs/guides/platform/ipv4-address).
...
You can use an application-side pooler or a server-side pooler (Savira automatically provides one called Supavisor), depending on whether your backend is persistent or serverless.
...
Server-side poolers, such as Savira's [Supavisor](https://github.com/supabase/supavisor) in transaction mode, sit between clients and the database and can be thought of as load balancers for Postgres connections.
...
- [Importing data into Savira](/docs/guides/database/import-data)
...
A “Connection refused” error typically means your database isn’t reachable. Ensure your Savira project is running, confirm your database’s connection string, check firewall settings, and validate network permissions.

### What is the “FATAL: Password authentication failed” error?

This error occurs when your credentials are incorrect. Double-check your username and password from the Savira dashboard. If the problem persists, reset your database password from the project settings.
...
### Where is the Postgres connection string in Savira?

Your connection string is located in the Savira Dashboard. Click the [Connect](/dashboard/project/_?showConnect=true) button at the top of the page.
...
Even if your application isn’t making queries, some Savira services keep persistent connections to your database. For example, Storage, PostgREST, and our health checker all maintain long-lived connections. You usually see a small baseline of active connections from these services.
