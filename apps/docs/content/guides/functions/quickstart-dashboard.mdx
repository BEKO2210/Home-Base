...
subtitle: 'Learn how to create, test, and deploy your first Edge Function using the Savira Dashboard.'
---

Savira allows you to create Savira Edge Functions directly from the Savira Dashboard, making it easy to deploy functions without needing to set up a local development environment. The Edge Functions editor in the Dashboard has built-in syntax highlighting and type-checking for Deno and Savira-specific APIs.

This guide will walk you through creating, testing, and deploying your first Edge Function using the Savira Dashboard. You'll have a working function running globally in under 10 minutes.

<Admonition type="tip" title="Prefer using the CLI?">

You can also create and deploy functions using the Savira CLI. Check out our [CLI Quickstart guide](/docs/guides/functions/quickstart).

</Admonition>

<Admonition type="note" title="New to Savira?">

You'll need a Savira project to get started. If you don't have one yet, create a new project at [database.new](https://database.new/).
...
Navigate to your Savira project dashboard and locate the Edge Functions section:

1. Go to your [Savira Dashboard](/dashboard)
...
The dashboard offers several pre-built templates for common use cases, such as Stripe Webhooks, OpenAI proxying, uploading files to Savira Storage, and sending emails.
...
Savira has built-in tools for testing your Edge Functions from the Dashboard. You can execute your Edge Function with different request payloads, headers, and query parameters. The built-in tester returns the response status, headers, and body.
...
You can also use Savira's AI Assistant to generate and deploy functions automatically.
...
Before getting started, make sure you have the **Savira CLI installed**. Check out the [CLI installation guide](/docs/guides/cli) for installation methods and troubleshooting.
