...
description: Learn how to configure your functions in Savira.
...
This configuration tell Savira that the `stripe-webhook` function doesn't require a valid JWT, the `image-processor` function uses a custom import map, and `legacy-processor` uses a custom entrypoint.
...
Be careful when using this flag, as it will allow anyone to invoke your Edge Function without a valid JWT. The Savira client libraries automatically handle authorization.
...
`entrypoint` is available only in Savira CLI version 1.215.0 or higher.
