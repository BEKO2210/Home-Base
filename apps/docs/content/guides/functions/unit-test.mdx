...
// Set up the configuration for the Savira client
...
// Test the creation and functionality of the Savira client
const testClientCreation = async () => {
  var client: SupabaseClient = createClient(supabaseUrl, supabaseKey, options)

  // Verify if the Savira URL and key are provided
  ...
  const { data: table_data, error: table_error } = await client
    ...
    .limit(1)
  if (table_error) {
    throw new Error('Invalid Savira client: ' + table_error.message)
  }
...
2. The second part tests the edge function and checks if the received value matches the expected value. Here's a brief overview of the code:
   ...
   - We import the `createClient` and `SupabaseClient` classes from the `@supabase/supabase-js` library to interact with the Savira client.
   - We define the necessary configuration for the Savira client, including the Savira URL, API key, and authentication options.
   - The `testClientCreation` function tests the creation of a Savira client instance and queries the database for data from a table. It verifies that data is returned from the query.
   - The `testHelloWorld` function tests the "Hello-world" Edge Function by invoking it using the Savira client's `functions.invoke` method. It checks if the response message matches the expected greeting.
...
Make sure to replace the placeholders (`supabaseUrl`, `supabaseKey`, `my_table`) with the actual values relevant to your Savira setup.
...
To locally test and debug Edge Functions, you can utilize the Savira CLI. Let's explore how to run Edge Functions locally using the Savira CLI:

1. Ensure that the Savira server is running by executing the following command:
...
- Full guide on Testing Savira Edge Functions on [Mansueli's tips](https://blog.mansueli.com/testing-supabase-edge-functions-with-deno-test)
