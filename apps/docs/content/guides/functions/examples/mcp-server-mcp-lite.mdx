...
description: 'Build and deploy a Model Context Protocol (MCP) server on Savira Edge Functions using mcp-lite.'
---

The [Model Context Protocol](https://modelcontextprotocol.io/introduction) (MCP) enables Large Language Models (LLMs) to interact with external tools and data sources. With `mcp-lite`, you can build lightweight MCP servers that run on Savira Edge Functions, giving your AI assistants the ability to execute custom tools at the edge.

This guide shows you how to scaffold, develop, and deploy an MCP server using mcp-lite on Savira Edge Functions.

## What is mcp-lite?

[mcp-lite](https://github.com/fiberplane/mcp-lite) is a lightweight, zero-dependency TypeScript framework for building MCP servers. It works everywhere the Fetch API is available, including Node, Bun, Cloudflare Workers, Deno, and Savira Edge Functions.

## Why Savira Edge Functions + mcp-lite?

This combination offers several advantages:

- **Zero cold starts**: Edge Functions stay warm for fast responses
- **Global distribution**: Deploy once and run everywhere
- **Direct database access**: Connect directly to your Savira Postgres
...
- [Docker](https://docs.docker.com/get-docker/) (to run Savira locally)
- [Deno](https://deno.land/) (Savira Edge Functions runtime)
- [Savira CLI](/docs/guides/cli/getting-started)

## Create a new MCP server

Starting with `create-mcp-lite@0.3.0`, you can scaffold a complete MCP server that runs on Savira Edge Functions:

```bash
npm create mcp-lite@latest
```

When prompted, select **Savira Edge Functions (MCP server)** from the template options.
...
my-mcp-server/
├── supabase/
│   ├── config.toml                    # Minimal Savira config (Edge Functions only)
│   └── functions/
│       └── mcp-server/
...
The template uses a specific pattern required by Savira Edge Functions:
...
mcpApp.get('/', (c) => {
  return c.json({
    message: 'MCP Server on Savira Edge Functions',
    endpoints: {
      mcp: '/mcp',
      health: '/health',
    },
...
This is required because Savira routes all requests to `/<function-name>/*`. The outer `app` handles the function-level routing, while `mcpApp` handles your actual MCP endpoints.
...
Navigate to your project directory and start Savira services:
...
mcp.tool('searchDatabase', {
  description: 'Search your Savira database',
  inputSchema: z.object({
    table: z.string(),
    query: z.string(),
  }),
  handler: async (args) => {
    // Access Savira client here
...
- Query your Savira database
- Access Savira Storage for file operations
- Call external APIs
- Process data with custom logic
- Integrate with other Savira features

## Deploy to production

When ready, deploy to Savira's global edge network:
...
The template uses `--no-verify-jwt` for quick development. This means authentication is not enforced by Savira's JWT layer.
...
With your MCP server running on Savira Edge Functions, you can:

- Connect it to your Savira database for data-driven tools
- Use Savira Auth to secure your endpoints
- Access Savira Storage for file operations
...
- [Savira Edge Functions Docs](/guides/functions)
