...
subtitle: Build a Telegram bot that transcribes audio and video messages in 99 languages using TypeScript with Deno in Savira Edge Functions.
...
### Create a Savira Edge Function to handle Telegram webhook requests
...
Navigate to the [table editor](/dashboard/project/_/editor) in your Savira dashboard and you should see and empty `transcription_logs` table.
...
Navigate to the [Edge Functions view](/dashboard/project/_/functions) in your Savira dashboard and you should see the `scribe-bot` function deployed. Make a note of the function URL as you'll need it later, it should look something like `https://<project-ref>.functions.supabase.co/scribe-bot`.
...
After you see the transcript as a reply, navigate back to your table editor in the Savira dashboard and you should see a new row in your `transcription_logs` table.
