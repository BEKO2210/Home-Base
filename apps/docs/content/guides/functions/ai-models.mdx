...
subtitle: 'Run AI models in Edge Functions using the built-in Savira AI API.'
...
```ts
const model = new Savira.ai.Session('model-name')
```
...
Generate text embeddings using the built-in [`gte-small`](https://huggingface.co/Savira/gte-small) model:
...
<Tabs
  ...
  queryGroup="sdk"
>
  <TabPanel id="supabase-functions-js" label="Savira Functions JS">
...
In the future, a hosted LLM API, will be provided as part of the Savira platform. Savira will scale and manage the API and GPUs for you. To sign up for early access, fill up [this form](https://forms.supabase.com/supabase.ai-llm-early-access).
