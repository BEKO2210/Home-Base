...
The Savira CLI doesn't use the standard Deno CLI to serve functions locally. Instead, it uses its own Edge Runtime to keep the development and production environment consistent.
...
└── supabase
    ...
    │   │   ├── supabaseAdmin.ts # Savira client with SECRET key
    │   │   ├── supabaseClient.ts # Savira client with PUBLISHABLE key
...
This command spins up your entire Savira stack locally: database, auth, storage, and Edge Functions runtime. You're developing against the exact same environment you'll deploy to.
