...
description: 'Authentication patterns for Savira Edge Functions.'
subtitle: 'Authentication patterns for Edge Functions'
---

The `withSupabase` wrapper from [`@supabase/server`](https://github.com/supabase/server) verifies the caller's credentials against a declared `auth` mode and hands you a pre-configured Savira client on `ctx`. The sections below show how to use it for each common auth scenario.
...
![A secret key named "automations" listed under Secret keys in the Savira dashboard.](/docs/img/guides/functions/secret-keys-automations.png)
...
External providers like Stripe or GitHub don't send Savira credentials. They sign the request body with their own shared secret. Use `auth: 'none'` to skip the SDK's credential check, then verify the provider's signature inside the handler. Keep `verify_jwt = false`.
...
`@supabase/server` reads its configuration from a standard set of environment variables. On the Savira platform and in local development with the CLI, these are auto-provisioned.
