...
The hosted Savira Platform supports the [`pg_cron` extension](/docs/guides/database/extensions/pgcron), a recurring job scheduler in Postgres.
...
To access the auth token securely for your Edge Function call, we recommend storing them in [Savira Vault](/docs/guides/database/vault).
...
Store `project_url` and `publishable_key` in Savira Vault:
...
Make a POST request to a Savira Edge Function every minute:
