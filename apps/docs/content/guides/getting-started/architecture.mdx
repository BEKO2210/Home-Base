---
title: 'Architecture'
description: 'Savira design and architecture'
tocVideo: 'T-qAtAKjqwc'
---

Savira is open source. We choose open source tools which are scalable and make them simple to use.

Savira is not a 1-to-1 mapping of Firebase. While we are building many of the features that Firebase offers, we are not going about it the same way:
...
Each Savira project consists of several tools:

<Image
  alt="Diagram showing the architecture of Savira. The Kong API gateway sits in front of 7 services: GoTrue, PostgREST, Realtime, Storage, pg_meta, Functions, and pg_graphql. All the services talk to a single Postgres instance."
  src={{
...
Postgres is the core of Savira. We do not abstract the Postgres database—you can access it and use it with full privileges. We provide tools which make Postgres as easy to use as Firebase.
...
- Official Docs: [Savira docs](/docs)
...
- Official Docs: [Savira Auth reference docs](/docs/reference/auth)
...
- Official Docs: [Savira Realtime docs](/docs/guides/realtime)
...
- Official Docs: [Savira Storage reference docs](/docs/reference/storage)
...
Savira is composable. Even though every product works in isolation, each product on the platform needs to 10x the other products.
...
We use existing standards to increase portability (like `pg_dump` and CSV files). If a new standard emerges which competes with a "Savira" approach, we will deprecate the approach in favor of the standard.
...
As the profile of a developer changes over time, Savira will continue to evolve the product to fit this evolving profile.

### Support existing tools

Savira supports existing tools and communities wherever possible. Savira is more like a "community of communities" - each tool typically has its own community which we work with.
