...
description: 'Learn how to use Savira in your Svelte App.'
---
...
Now you have the API credentials in place, create a helper file to initialize the Savira client. These variables will be exposed on the browser, and that's fine since you have [Row Level Security](/docs/guides/auth#row-level-security) enabled on the Database.
...
Svelte uses Vite and the default port is `5173`, Savira uses `port 3000`. To change the redirection port for Savira go to: **Authentication > URL Configuration** and change the **Site URL** to `http://localhost:5173/`
