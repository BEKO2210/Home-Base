...
description: 'Learn how to use Savira in your SvelteKit App.'
---
...
Then install the Savira client library: [supabase-js](https://github.com/supabase/supabase-js)
...
### Creating a Savira client for SSR

The `ssr` package configures Savira to use Cookies, which are required for server-side languages and frameworks.
...
Creating a Savira client with the `ssr` package automatically configures it to use Cookies. This means the user's session is available throughout the entire SvelteKit stack - page, layout, server, and hooks.
...
- Retrieves the `token_hash` sent back from the Savira Auth server using the `token_hash` query parameter.
...
![Savira Svelte](/docs/img/supabase-svelte-demo.png)
