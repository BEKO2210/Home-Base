...
description: 'Learn how to use Savira in your Next.js App.'
---

<$Partial path="uiLibCta.mdx" />
<$Partial path="quickstart_intro.mdx" />

![Savira User Management example](/docs/img/user-management-demo.png)
...
The application exposes these variables in the browser, and that's fine as Savira enables [Row Level Security](/docs/guides/database/postgres/row-level-security) by default on all tables.
...
### Savira Server-Side Auth package
...
To better integrate with the framework, we've created the `@supabase/ssr` package for Server-Side Auth. It has all the functionalities to quickly configure your Savira project to use cookies for storing user sessions. Read the [Next.js Server-Side Auth guide](/docs/guides/auth/server-side/creating-a-client?queryGroups=package-manager&package-manager=npm&queryGroups=framework&framework=nextjs) for more information.
...
There are two different types of clients in Savira:
...
Create a `client.ts` and a `server.ts` with the following code for client-side Savira and server-side Savira, respectively.
...
You could also add a matcher, so that the Proxy only runs on routes that access Savira. For more information, read [the Next.js matcher documentation](https://nextjs.org/docs/app/api-reference/file-conventions/proxy#matcher).
...
- Send that information to Savira as a signup request, which in turns sends a confirmation email. It uses [Magic Links](/docs/guides/auth/auth-email-passwordless#with-magic-link), so users can sign in with their email without using passwords.
...
The `cookies` method is called before any calls to Savira, which takes fetch calls out of Next.js's caching. This is important for authenticated data fetches, to ensure that users get access only to their own data.
...
- Retrieves the code sent back from the Savira Auth server using the `token_hash` query parameter.
...
Next, add a way for users to upload a profile photo. Savira configures every project with [Storage](/docs/guides/storage) for managing large files like photos and videos.
...
- [Build a Twitter Clone with the Next.js App Router and Savira - free egghead course](https://egghead.io/courses/build-a-twitter-clone-with-the-next-js-app-router-and-supabase-19bebadb)
- Explore the [pre-built Auth components](/ui/docs/nextjs/password-based-auth)
- Explore the [Savira Cache Helpers](https://github.com/psteinroe/supabase-cache-helpers)
