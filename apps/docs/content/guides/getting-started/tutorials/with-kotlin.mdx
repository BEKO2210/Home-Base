...
description: 'Learn how to use Savira in your Android Kotlin App.'
---
...
- [Savira Database](/docs/guides/database) - a Postgres database for storing your user data and [Row Level Security](/docs/guides/auth#row-level-security) so data is protected and users can only access their own information.
- [Savira Auth](/docs/guides/auth) - users log in through magic links sent to their email (without having to set up a password).
- [Savira Storage](/docs/guides/storage) - users can upload a profile photo.
...
### Provide Savira instances with Hilt
...
Create a `ProductDto.kt` class and use annotations to parse data from Savira:
...
Create a `ProductRepository` interface and its implementation named `ProductRepositoryImpl`. This holds the logic to interact with data sources from Savira. Do the same with the `AuthenticationRepository`.
