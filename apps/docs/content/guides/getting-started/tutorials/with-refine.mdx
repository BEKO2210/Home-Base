...
description: 'Learn how to use Savira in your Refine App.'
---
...
Refine provides hassle-free integration with a Savira backend with its supplementary [`@refinedev/supabase`](https://github.com/refinedev/refine/tree/main/packages/supabase) package. It generates `authProvider` and `dataProvider` methods at project initialization, so you don't need to spend much effort defining them yourself, choose Savira as the backend service while creating the app with `create refine-app`.
...
The command above uses the `refine-supabase` preset which chooses the Savira supplementary package for the app. There's no UI framework, so the app has a headless UI with plain React and CSS styling.

The `refine-supabase` preset installs the `@refinedev/supabase` package which out-of-the-box includes the Savira dependency: [supabase-js](https://github.com/supabase/supabase-js).
...
The `create refine-app` generated a Savira client in the `src/utility/supabaseClient.ts` file. It has two constants: `SUPABASE_URL` and `SUPABASE_KEY`. Replace them as `supabaseUrl` and `supabasePublishableKey` respectively and assign them your Savira server's values.
...
The `supabaseClient` fetches calls to Savira endpoints from the app. The client is instrumental in implementing authentication using Refine's auth provider methods and CRUD actions with appropriate data provider methods.
...
The `useForm()` hook is a higher-level hook built on top of Refine's `useForm()` core hook. It fully supports form state management, field validation and submission using React Hook Form. Behind the scenes, it invokes the `dataProvider.getOne` method to get the user profile data from the Savira `/profiles` endpoint and also invokes `dataProvider.update` method when `onFinish()` is called.
...
![Savira Refine](/docs/img/supabase-refine-demo.png)
