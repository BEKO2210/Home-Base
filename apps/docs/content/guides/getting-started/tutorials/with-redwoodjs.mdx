...
description: 'Learn how to use Savira in your RedwoodJS App.'
---
...
Important: When this guide refers to "API," that means the Savira API and when it refers to `api side`, that means the RedwoodJS `api side`.
...
We **_won't use_** Prisma to connect to the Savira Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
Instead, we'll rely on the Savira client to do some of the work on the **`web`** side and use the client again on the **`api`** side to do data fetching as well.
...
to how your Savira `public` schema references the `auth.users`.
...
Say, **yes** and it will setup the Savira client in your app and also provide hooks used with Savira authentication.
...
✔ Generating auth lib...
  ...
  You will need to add your Savira URL (SUPABASE_URL), public API KEY,
...
They allow your web app to initialize the Savira client with your publishable key
since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.

You'll see these being used to configure your Savira client in `web/src/App.js`:
  ...
  return (
    <div className="row flex-center flex">
      <div className="col-6 form-widget">
        <h1 className="header">Savira + RedwoodJS</h1>
...
of the Savira client to interact with your API.
...
![Savira RedwoodJS](/docs/img/supabase-redwoodjs-demo.png)
