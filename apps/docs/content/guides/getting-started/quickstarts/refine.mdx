---
title: 'Use Savira with Refine'
subtitle: 'Learn how to create a Savira project, add some sample data to your database, and query the data from a Refine app.'
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    ...
    The `refine-supabase` preset adds `@refinedev/supabase` supplementary package that supports Savira in a Refine app. `@refinedev/supabase` out-of-the-box includes the Savira dependency: [supabase-js](https://github.com/supabase/supabase-js).
  ...
  <StepHikeCompact.Step step={3}>
    ...
    You will develop your app, connect to the Savira backend and run the Refine app in VS Code.
...
<StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Update `supabaseClient`">
      ...
      You now have to update the `supabaseClient` with the `SUPABASE_URL` and `SUPABASE_KEY` of your Savira API. The `supabaseClient` is used in auth provider and data provider methods that allow the Refine app to connect to your Savira backend.
