---
title: 'Use Savira with Laravel'
subtitle: 'Learn how to create a PHP Laravel project, connect it to your Savira Postgres database, and configure user authentication.'
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    By default Laravel uses the `public` schema. We recommend changing this as Savira exposes the `public` schema as a [data API](/docs/guides/api).

    You can change the schema of your Laravel application by modifying the `search_path` variable `app/config/database.php`.

    The schema you specify in `search_path` has to exist on Savira. You can create a new schema from the [Table Editor](/dashboard/project/_/editor).
  ...
  <StepHikeCompact.Step step={5}>
    ...
    Note: Laravel does not use Savira Auth but rather implements its own authentication system!
