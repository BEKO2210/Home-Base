...
description: 'Set up SAML 2.0 Single Sign-On for self-hosted Savira with Docker.'
subtitle: 'Set up SAML 2.0 Single Sign-On for self-hosted Savira with Docker.'
---
...
This guide covers the full setup: generating a signing key, enabling SAML in your Savira instance, registering an IdP, and integrating SSO into your application.
...
- A running self-hosted Savira instance (see the [setup guide](/docs/guides/self-hosting/docker))
...
- `API_EXTERNAL_URL` set to the publicly-accessible URL of your Savira Auth service (e.g., `https://<your-domain>`). This URL is used as the SAML Service Provider entity ID and for constructing the ACS endpoint URL

## How SAML SSO works in Savira
...
Once SAML is enabled, your Savira instance exposes service provider (SP) metadata at `{API_EXTERNAL_URL}/sso/saml/metadata`.
...
Attribute mapping lets you control how SAML assertion attributes are translated into Savira user claims. If no mapping is provided, Auth uses sensible defaults:
