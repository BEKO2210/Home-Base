...
description: 'Set up a reverse proxy with HTTPS for self-hosted Savira.'
subtitle: 'Set up a reverse proxy with HTTPS for self-hosted Savira.'
---

HTTPS is required for production self-hosted Savira deployments. This guide covers two production approaches using a reverse proxy in front of self-hosted Savira API gateway, plus a self-signed certificate option for development environment.
...
Below are two options for adding a reverse proxy with automatic HTTPS in front of your self-hosted Savira: **Caddy** (simpler, zero-config TLS) and **Nginx + Let's Encrypt** (more control over proxy settings). Both sit in front of the API gateway and terminate TLS, so internal traffic stays on HTTP.
