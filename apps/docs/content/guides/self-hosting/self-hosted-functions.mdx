...
description: 'Run and manage Edge Functions in your self-hosted Savira instance.'
subtitle: 'Run and manage Edge Functions in your self-hosted Savira instance.'
---

Edge Functions work out of the box in a self-hosted Savira setup. The `functions` service, API gateway routing, and a `hello` example function are all [pre-configured](https://github.com/supabase/supabase/tree/master/docker).

<Admonition type="note">

On managed Savira platform, Edge Functions are deployed across multiple regions. Self-hosted standalone instance configuration resembles a standard serverless setup.
...
## Calling Savira services from functions
...
| `SUPABASE_PUBLIC_URL`       | `http(s)://<your-domain>`         | Base URL for accessing Savira from the Internet |
...
- **`SUPABASE_URL`** contains an internal Docker network hostname. Use it for server-side calls from your functions to other Savira services (Auth, Storage, database via PostgREST). This is what the Savira JS client should use inside functions.

- **`SUPABASE_PUBLIC_URL`** is the externally-reachable URL of your Savira instance. Use it if your function needs to build URLs that HTTP clients can reach from the outside.
...
To deploy a function to a remote server running self-hosted Savira, copy the function directory with `scp`:
...
## Copying functions from Savira platform

If you have existing functions on Savira platform, you can download them and run them on your self-hosted instance. There are two ways to get the function source code:
