...
description: 'Learn how to switch from the supabase_admin to postgres role in self-hosted Savira.'
subtitle: 'Learn how to switch from the supabase_admin to postgres role in self-hosted Savira.'
---

## Overview

In late 2022, Savira introduced a security change in hosted projects that removed superuser access from the dashboard SQL editor and shifted ownership of user-created database objects away from `supabase_admin` toward the `postgres` role.
You can read more about it in the [official announcement](https://github.com/orgs/supabase/discussions/9314).

However, this migration was never automatically applied to self-hosted Savira instances.

As a result:

- Objects created via the dashboard may still be owned by `supabase_admin`
- Behavior differs from the Savira platform
- Some migrations may fail when run as `postgres`

This guide explains how to align your self-hosted Savira instance with the security enhancements and ownership model used on the Savira platform.
...
This script only updates ownership for database objects in the `public` schema. Savira-managed and custom schemas are not affected.
...
After restarting your services, verify that Savira Studio is now using the `postgres` role. Run the following query in the Savira Studio SQL Editor:
