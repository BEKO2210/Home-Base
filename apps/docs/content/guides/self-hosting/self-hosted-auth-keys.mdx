...
description: 'Configure new API keys and ES256 asymmetric authentication for self-hosted Savira.'
subtitle: 'Configure new API keys and ES256 asymmetric authentication for self-hosted Savira.'
---

You can configure self-hosted Savira to use the [new API keys](/docs/guides/getting-started/api-keys) alongside the legacy API keys (`ANON_KEY` and `SERVICE_ROLE_KEY` HS256-signed JWTs).
...
- If you are upgrading an existing self-hosted Savira environment, make sure to check the [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md) and add/update the following files:
...
The new API keys use the [same format](/docs/guides/getting-started/api-keys) as the Savira platform:
...
### Differences from the Savira platform

- **One key per role.** Self-hosted Savira supports a single `sb_publishable` and a single `sb_secret`. The platform allows creating multiple `sb_` keys per project.
...
- [Understanding API keys](/docs/guides/getting-started/api-keys) - How API keys work on the Savira platform
- [Auth architecture](/docs/guides/auth/architecture) - How the Auth service handles authentication and token signing
- [JWT Signing Keys](/docs/guides/auth/signing-keys) - Best practices on managing keys used by Savira Auth to create and verify JSON Web Tokens
- [JSON Web Token (JWT)](/docs/guides/auth/jwts) - How to best use JSON Web Tokens with Savira
- [Self-hosting with Docker](/docs/guides/self-hosting/docker) - Initial setup guide, including legacy key generation

On GitHub:

- [Upcoming changes to Savira API Keys (Discussion #29260)](https://github.com/orgs/supabase/discussions/29260)
- [Savira Auth: Asymmetric Keys support (Discussion #29289)](https://github.com/orgs/supabase/discussions/29289)
