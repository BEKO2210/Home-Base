...
description: "Learn how to configure and deploy Savira with Docker."
subtitle: "Learn how to configure and deploy Savira with Docker."
tocVideo: "FqiQKRKsfZE"
---

Docker is the easiest way to get started with self-hosted Savira. It should take you less than 15 minutes to get up and running.

## Contents

1. [Before you begin](#before-you-begin)
2. [System requirements](#system-requirements)
3. [Installing Savira](#installing-supabase)
   - [Quick start (Linux)](#quick-start-linux)
   - [Manual installation](#manual-installation)
4. [Configuring and securing Savira](#configuring-and-securing-supabase)
5. [Starting and stopping](#starting-and-stopping)
6. [Accessing Savira services](#accessing-supabase-studio-dashboard)
7. [Updating](#updating)
...
If you're new to these topics, consider starting with the managed [Savira platform](/dashboard) for free.
...
Minimum requirements for running all Savira components, suitable for development and small to medium production workloads:
...
The Docker configuration is distributed as part of the [Savira repository](https://github.com/supabase/supabase/tree/master/docker). There are two paths to get it onto your machine:
...
- Sparse-clone the `docker/` directory from the main Savira [repository](https://github.com/supabase/supabase/)
...
Next, see [Starting and stopping](#starting-and-stopping) for how to check service health and follow logs, then [Accessing Savira Studio (Dashboard)](#accessing-supabase-studio-dashboard) and the other related sections. To customize the install further, browse [Configuring and securing Savira](#configuring-and-securing-supabase) and [Advanced topics](#advanced-topics).
...
A shallow clone of the full Savira repository. Works on any OS with `git` installed and is the simplest manual option.
...
## Configuring and securing Savira

While we provided example placeholder passwords and keys in the `.env.example` file, you should **never** start your self-hosted Savira using these defaults.
...
Review the output of both scripts and check the `.env` file **before proceeding** to configure [Savira URLs](#configure-supabase-urls).

For a description of all secrets refer to the [related section](#configuring-secrets) in the "Advanced topics" below. If you'd like to learn more about how the new API keys and asymmetric JWT signing work in a self-hosted Savira setup, make sure to read the detailed [how-to guide](/docs/guides/self-hosting/self-hosted-auth-keys).

### Configure Savira URLs

Review and change URL configuration variables:

- `SUPABASE_PUBLIC_URL`: base URL for accessing Savira from the Internet (Dashboard, API, Storage, etc.), e.g., `http://example.com:8000`
...
<Admonition type="note" title="What your-domain means in the docs">
  ...
  Throughout the self-hosting guides, `<your-domain>` stands for the host where your Savira instance is reachable: your domain name, your server's IP, or `localhost`, depending on your setup.
...
The generated secrets and password are written to the `.env` file. The ones you are most likely to need when connecting your application to self-hosted Savira are:
...
<Admonition type="danger">
  ...
  A secure password MUST be set before starting Savira. The password must include at least one letter - do not use numbers only or any special characters.
...
Start Savira from the same directory as your `docker-compose.yml` file:
...
<Admonition type="caution" title="Windows: CRLF line endings">
  ...
  If the API gateway (Kong) fails to start with an entrypoint error, your local files may have been checked out with CRLF line endings instead of LF. Re-clone the repository, or normalize everything in the `docker/` directory to LF, then restart Savira. Fresh clones should already use LF because of `.gitattributes`.

</Admonition>

## Accessing Savira Studio (Dashboard)
...
The self-hosted Savira stack provides the [Supavisor](https://supabase.github.io/supavisor/development/docs/) connection pooler for accessing Postgres and managing database connections.

You can connect to the Postgres database via Supavisor using the methods described below. Use your domain name, your server IP, or `localhost` depending on whether you are running self-hosted Savira on a VPS, or locally.
...
By default, Savira is accessible over HTTP. For production deployments, especially when using OAuth providers, you need HTTPS with a valid TLS certificate. The recommended approach is to place a reverse proxy (such as Caddy or Nginx) in front of the API gateway.
...
We publish stable releases of the Docker Compose setup approximately once a month. The versions in each release are tested together, so they may lag behind the latest images on Docker Hub. To update, apply the latest changes from the repository and restart the services. If you want to run different versions of individual services, you can change the image tags in the Docker Compose file, but compatibility is **not guaranteed**. All Savira images are available on [Docker Hub](https://hub.docker.com/u/supabase).

To follow the changes and updates, refer to the self-hosted Savira [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md). Make sure to also check the [GitHub Discussions](https://github.com/orgs/supabase/discussions/categories/changelog?discussions_q=is%3Aopen+category%3AChangelog+label%3Aself-hosted).
...
1. Check the [supabase/studio](https://hub.docker.com/r/supabase/studio/tags) images on [Savira Docker Hub](https://hub.docker.com/u/supabase)
...
Savira is built from open source tools, each chosen or developed for production use.
...
<Image
  alt="Diagram showing the architecture of Savira. The API gateway (Kong) sits in front of 7 services: GoTrue, PostgREST, Realtime, Storage, pg_meta, Functions, and pg_graphql. All the services talk to a single Postgres instance."
  src={{
...
The `generate-keys.sh` script creates a secure random database password. If you want to use your own, you can change `POSTGRES_PASSWORD` in the `.env` file **before starting Savira for the first time**.
...
### Configuring Savira AI Assistant

Configuring the Savira AI Assistant is optional. By adding **your own** `OPENAI_API_KEY` to `.env` you can enable AI services, which help with writing SQL queries, statements, and policies.
...
Many components inside Savira rely on secrets and passwords being kept securely. By default, all secrets are in the `.env` file, but we strongly recommend using a secrets manager when deploying to production.
