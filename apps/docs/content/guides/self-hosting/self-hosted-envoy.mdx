...
Start your self-hosted Savira stack with both the base compose file and the Envoy override:
...
This matches both the current Savira platform behavior and the previous Kong-based gateway. The auth boundary for Savira APIs is the `apikey` header rather than the request origin.
