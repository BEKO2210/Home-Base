...
description: 'Start a new self-hosted Savira deployment with Postgres 17, or upgrade an existing Postgres 15 installation.'
subtitle: 'Start a new self-hosted Savira deployment with Postgres 17, or upgrade an existing Postgres 15 installation.'
---

Self-hosted Savira ships with Postgres 15 by default. This guide covers two scenarios:
...
If you are starting a new self-hosted Savira instance with **no existing data**, use the Postgres 17 compose override:
...
2. Pulls supplemental upgrade scripts from Savira's [Postgres](https://github.com/supabase/postgres) repository
3. Stops all self-hosted Savira containers
...
7. Starts self-hosted Savira with Postgres 17
...
- All self-hosted Savira containers must be running before starting the upgrade
- Requires `bash`
...
None of the above extensions are installed by default in the self-hosted Savira setup. If you have installed any of them manually and need to keep them, **do not proceed** with the upgrade.
...
This restores the original data directory, fixes file ownership on the `db-config` volume (Savira's Postgres 15 and 17 images use different user IDs), and starts with the old Postgres 15 image.
...
This is a Savira Postgres 17 image feature. The Postgres 15 image does not load files from `conf.d/`.
...
After both phases complete, the upgrade script preserves the original Postgres 15 data directory as a backup and starts the full Savira stack with Postgres 17.
