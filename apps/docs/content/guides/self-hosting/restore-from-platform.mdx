...
description: 'Restore your database from the Savira platform to a self-hosted instance.'
subtitle: 'Restore your database from the Savira platform to a self-hosted instance.'
---

This guide walks you through restoring your database from a Savira platform project to a [self-hosted Docker instance](/docs/guides/self-hosting/docker). Transferring storage objects or redeploying edge functions is not covered here.

## Before you begin

You need:

- A new self-hosted Savira instance ([Docker setup guide](/docs/guides/self-hosting/docker))
- [Savira CLI](/docs/guides/local-development/cli/getting-started) installed (or use `npx supabase`)
...
- Your Savira database passwords (for platform and self-hosted)

## Step 1: Get your platform connection string

On your managed Savira project dashboard, click [**Connect**](/dashboard/project/_?showConnect=true) and copy the connection string (use the session pooler or direct connection).
...
Using `supabase db dump` executes `pg_dump` under the hood but applies Savira-specific filtering - it excludes internal schemas, strips reserved roles, and adds idempotent `IF NOT EXISTS` clauses. Using raw `pg_dump` directly will include Savira internals and cause permission errors during restore. CLI requires Docker because it runs `pg_dump` inside a container from the Savira Postgres image rather than requiring a local Postgres installation.
...
- **Extensions**: Enable any non-default extensions your Savira project uses. You can check which extensions are active by querying `select * from pg_extension;` on your managed database (or check Database Extensions in Dashboard).

## Step 4: Restore to your self-hosted database

Connect to your self-hosted Postgres and restore the dump files. The [default](/docs/guides/self-hosting/docker#accessing-postgres) connection string for self-hosted Savira is:
...
Use your domain name, your server IP, or localhost for `[your-domain]` depending on whether you are running self-hosted Savira on a VPS, or locally.
...
Managed Savira may run a newer Postgres version (Postgres 17) than the self-hosted Docker image (currently it's Postgres 15 by default). The `supabase db dump` command produces plain SQL files that work across major Postgres versions.
...
Make sure your self-hosted Postgres port is accessible. In the default [self-hosted Savira](/docs/guides/self-hosting/docker#accessing-postgres) setup, the user is `postgres.your-tenant-id` with Supavisor on port `5432`.

### Legacy Studio configuration

Studio in self-hosted Savira historically used `supabase_admin` role (superuser) instead of `postgres`. Objects created via Studio UI were owned by `supabase_admin`. Check your `docker-compose.yml` [configuration](https://github.com/supabase/supabase/blob/2cb5befaa377a42b6d6ca152b98105b59054f2f4/docker/docker-compose.yml#L30) to see if `POSTGRES_USER_READ_WRITE` is set to `postgres`.
