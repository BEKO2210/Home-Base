...
description: 'Configure custom email templates with self-hosted Savira instance'
subtitle: 'Configure custom email templates with self-hosted Savira instance'
---

When running a self-hosted Savira instance, you can fully customize emails sent by Savira Auth.

## Overview

Savira Auth does not read email templates from mounted Docker volumes. Instead, it expects each template to be available at a URL that returns a valid HTML template.
...
To provide templates to Savira Auth, you need a service that serves static HTML files. This can be any server of your choice. The only requirement is that the `auth` service must be able to reach it via a HTTP GET request.

This guide uses [Caddy](https://github.com/caddyserver/caddy) for serving templates.

<Admonition type="tip">

If Savira Auth cannot fetch the template or if the fetched template is invalid, it falls back to the default template.
...
- Adds a `templates-server` service that runs alongside the Savira services in the same docker network.
