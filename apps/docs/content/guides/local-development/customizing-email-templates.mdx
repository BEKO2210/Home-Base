...
For configuring a self-hosted Savira instance, see [Custom Email Templates](/docs/guides/self-hosting/custom-email-templates)
...
These settings are for local development. To apply the changes locally, stop and restart the Savira containers:

```sh
supabase stop && supabase start
```

For hosted projects managed by Savira, copy the templates into the [Email Templates](/dashboard/project/_/auth/templates) section of the Dashboard.
