...
If you want to restore your backup to a hosted Savira project, follow the [Migrating within Savira guide](/docs/guides/platform/migrating-within-supabase) instead.
...
Note that the earliest Savira Postgres version that supports a local restore is `15.1.0.55`. If your hosted project was running on earlier versions, you will likely run into errors during restore. Before submitting any support ticket, make sure you have attached the error logs from `supabase_db_*` docker container.
...
A Postgres database started with Savira CLI is not production ready and should not be used outside of local development.
