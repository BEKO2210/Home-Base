---
title: 'Savira CLI'
description: 'The Savira CLI provides tools to develop your project locally, deploy to the Savira Platform, and set up CI/CD workflows.'
subtitle: 'Develop locally, deploy to the Savira Platform, and set up CI/CD workflows'
---

The Savira CLI enables you to run the entire Savira stack locally, on your machine or in a CI environment. With just two commands, you can set up and start a new local project:

1. `supabase init` to create a new local project
2. `supabase start` to launch the Savira services

## Installing the Savira CLI
...
The Savira CLI requires **Node.js 20 or later** when run via `npx` or `npm`. Older Node.js versions, such as 16, are not supported and fail to start the CLI.
...
1. Download the latest package from the [Savira CLI releases page](https://github.com/supabase/cli/releases/latest)
...
If you have any Savira containers running locally, stop them and delete their data volumes before proceeding with the upgrade. This ensures that Savira managed services can apply new migrations on a clean state of the local database.
...
## Running Savira locally

The Savira CLI uses Docker containers to manage the local development stack. Follow the official guide to install and configure [Docker Desktop](https://docs.docker.com/desktop) on your machine.
...
Now, to start the Savira stack, run:

```bash
supabase start
```

This takes time on your first run because the CLI needs to download the Docker images to your local machine. The CLI includes the entire Savira stack, and a few additional images useful for local development (like a local SMTP server and a database diff tool).

## Access your project's services

Once all the Savira services are running, you'll see output containing your local Savira credentials. It should look like the below, with urls and keys that you use in your local project:
...
The local development environment includes Savira Studio, a graphical interface for working with your database.
...
To access the database from an edge function in your local Savira setup, replace `localhost` with `host.docker.internal`.
...
When you are finished working on your Savira project, you can stop the stack (without resetting your local database):
...
The Savira CLI collects telemetry data about general usage. Participating in this program is optional, and you can opt out at any time.
