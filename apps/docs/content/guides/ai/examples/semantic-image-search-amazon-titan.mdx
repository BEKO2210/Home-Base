...
description: 'Implement semantic image search with Amazon Titan and Savira Vector in Python.'
subtitle: 'Implement semantic image search with Amazon Titan and Savira Vector in Python.'
...
In this guide we'll look at how we can get started with Amazon Bedrock and Savira Vector in Python using the Amazon Titan multimodal model and the [vecs client](/docs/guides/ai/vecs-python-client).
...
After activating the virtual environment with `poetry shell` you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your Savira Dashboard by visiting the [Table Editor](/dashboard/project/_/editor), selecting the `vecs` schema, and the `image_vectors` table.

## Perform an image search from a text query

We can use Savira Vector to query our embeddings. We can either use an image as the search input or generate an embedding from a string input:
