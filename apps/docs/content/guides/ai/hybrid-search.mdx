...
In practice, you will likely be calling this from the [Savira client](/docs/reference/javascript/introduction) or through a custom backend layer. Here is a quick example of how you might call this from an [Edge Function](/docs/guides/functions) using JavaScript:
...
Deno.serve(async (req) => {
  ...
  // Instantiate the Savira client
  // (replace service role key with user's JWT if using Savira auth and RLS)
