...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    _Optionally_ passing custom configuration to `Savira.initialize()` is now organized into separate objects:
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    With the new update, developers can use Savira and Provider in the same codebase without any import prefixes.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    In v1, `Savira.initialize()` would await for the session to be refreshed before returning.
    This caused delays in the app's launch time, especially when the app is opened in a poor network environment.

    In v2, `Savira.initialize()` returns immediately after obtaining the session from the local storage, which makes the app launch faster.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    `Savira.initialize()` no longer has the `authCallbackUrlHostname` parameter.
