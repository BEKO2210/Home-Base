...
- Update dependencies: `functions-csharp@1.3.1`, `gotrue-csharp@4.0.4`, `postgrest-csharp@3.2.2`,
  ...
  - Namespaces assembly names to make them unique among other dependencies, i.e: `Core.dll`
    becomes `Savira.Core.dll` which will hopefully prevent future collisions.
...
- Changes options to require `Savira.SupabaseOptions.SessionPersistor` from using `ISupabaseSessionHandler`
...
- Add support for functions-csharp@1.0.1, giving access to invoking Savira's edge functions.
...
- Separate Storage client from Savira repo and into `storage-csharp`, `supabase-csharp` now references new repo.
