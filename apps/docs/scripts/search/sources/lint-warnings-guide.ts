...
export class LintWarningsGuideSource extends BaseSource {
  ...
  extractIndexedContent(): string {
    ...
    return `# Database Advisor: Lint ${this.lint.path}\n\nThis is a database lint rule for Savira, targeting the lint ID ${this.lint.path}. Lint rules help enforce performance and security best practices for your Savira database.\n\n${sectionText}`
  }
}
