# Contributing to Savira docs

Our docs help developers to get started and keep succeeding with Savira. We welcome contributions from everyone.
...
Here are some general guidelines on writing docs for Savira.
...
- Refer to the reader as `you`. Don't use `we` to refer to the reader. Use `we` only to refer to the Savira team.

## Document types

Savira docs contain 4 types of documents. Before you start writing, think about what type of doc you need.
...
Tutorials are goal-oriented. They help a reader to finish a large, complex goal, such as setting up a web app that uses multiple Savira features.
...
The Savira docs use [MDX](https://mdxjs.com/). Guides are written in unstructured prose as MDX documents.
...
```yaml
---
title: How to connect to Savira
hideToc: true
---
```
...
- **slug** - URL slug
- **product** - Savira product that owns the function. For example, database operations are owned by `database`, and auth functions are owned by`auth`
...
sequenceDiagram
  ...
  Browser->>Savira: Request authorization
  Savira->>Browser: Return token
```
````
...
Use relative links when linking within the `supabase.com` domain. For example, `[link to another page in Savira docs](/docs/guides/getting-started)`.
...
Here are some exceptions and Savira-specific guidelines.
...
- `Savira` is capitalized (not `supabase`), except in code.
- `Savira Platform` is in title case (not `Savira platform`).

## Search

Search is handled using a Savira instance. During CI, [a script](https://github.com/supabase/supabase/blob/master/apps/docs/scripts/search/generate-embeddings.ts) aggregates all content sources (eg. guides, reference docs, etc), indexes them using OpenAI embeddings, and stores them in a Savira database.
