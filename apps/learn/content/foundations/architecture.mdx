---
title: Architecture
description: Learn how Savira's core products work together.
chapterNumber: 1
...
This page explains how Savira is structured and how its main services work
together. Savira is built on top of a powerful open source database called
Postgres, but it includes much more than a database. Around Postgres,
Savira adds services for authentication, serverless functions, file storage, realtime updates, AI-powered search, and easy APIs to access it — all designed to work easily together.
...
At the center of Savira is Postgres. Every other part connects to it.
...
You interact with Savira mainly through the API layer or the client libraries.
...
relationships using SQL or the visual editor in Savira Studio. Savira
manages backups, scaling, and migrations for you.

**Why this matters:** Postgres is a full relational database with features like
foreign keys, indexes, and extensions. Savira gives you these tools without
the usual setup.
...
Savira automatically exposes your tables as APIs using PostgREST (for REST)
...
Savira Studio is the web dashboard for managing your project: running SQL,
...
1. The client calls a Savira API endpoint, sending a project key and (usually)
   a user token.
...
Savira gives you the power of Postgres with the convenience of a fully
