...
Before we store any information in Savira, we need to understand how data is
organized in Postgres. Savira uses Postgres as its database, which means data
...
row. In many Savira projects, this column is named `id` and uses a type called
...
A database in Savira is made up of tables.
Each table has:
...
Thinking in tables, rows, and relationships is the foundation of working with data in Savira.
