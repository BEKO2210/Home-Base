...
const loginWithGithub = async ({
  ...
  try {
    ...
    // Wait for redirect after 2FA - either to authorization page or directly to Savira
    console.log('Waiting for redirect after 2FA...')
    await page.waitForURL(
      (url) => {
        const href = url.href
        // Either GitHub authorization page or redirect to Savira
        return (
    ...
    if (needsAuthorization) {
      ...
      console.log('Clicked Authorize button, waiting for redirect to Savira...')

      // Wait for redirect to Savira after clicking authorize
      await page.waitForURL(
        (url) =>
      ...
      console.log('No authorization needed, already on Savira')
    }
